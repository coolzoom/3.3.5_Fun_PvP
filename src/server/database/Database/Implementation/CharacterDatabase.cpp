...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(CHAR_SEL_GROUP_INSTANCE, "SELECT guid FROM group_instance WHERE instance = ?", CONNECTION_BOTH);
    ...
    PrepareStatement(CHAR_SEL_ITEM_LEVEL, "SELECT itemLevel FROM item_instance WHERE guid = ? LIMIT 1", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_ITEM_INSTANCE_BYOWNER, "SELECT guid FROM item_instance WHERE owner_guid = ? AND itemEntry = ? LIMIT 1", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_REP_ITEM_INSTANCE, "REPLACE INTO item_instance (itemEntry, itemLevel, owner_guid, creatorGuid, giftCreatorGuid, count, duration, charges, flags, enchantments, randomPropertyId, durability, playedTime, text, guid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_ITEM_INSTANCE, "UPDATE item_instance SET itemEntry = ?, itemLevel = ?, owner_guid = ?, creatorGuid = ?, giftCreatorGuid = ?, count = ?, duration = ?, charges = ?, flags = ?, enchantments = ?, randomPropertyId = ?, durability = ?, playedTime = ?, text = ? WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_ITEM_INSTANCE_LEVEL, "UPDATE item_instance SET itemLevel = ? WHERE guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHANNEL, "SELECT announce, ownership, bannedList FROM channels WHERE name = ? AND team = ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_INS_CHANNEL, "INSERT INTO channels(name, team, lastUsed) VALUES (?, ?, UNIX_TIMESTAMP())", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_CHANNEL, "UPDATE channels SET announce = ?, ownership = ?, bannedList = ?, lastUsed = UNIX_TIMESTAMP() WHERE name = ? AND team = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_CHAR_RAIDINFO, "INSERT INTO character_raidinfo (GuildId, GuildName, CreatureEntry, PlayerCount, LeaderName, RaidId, KillData, MapSpawnMode, MapId, CreatureName, MapName, MapMaxPlayers, CompletedEncounters, TotalEncounters) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
