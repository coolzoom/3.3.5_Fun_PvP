...
void LoginDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(LOGIN_SEL_IP_COUNT, "SELECT COUNT(last_ip) FROM account WHERE last_ip IN (SELECT last_ip FROM account WHERE id = ?) AND online = 1", CONNECTION_SYNCH);
