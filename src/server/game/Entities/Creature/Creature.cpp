...
bool Creature::Create(ObjectGuid::LowType guidlow, Map* map, uint32 phaseMask, uint32 entry, float x, float y, float z, float ang, CreatureData const* data /*= nullptr*/, uint32 vehId /*= 0*/)
{
    ...
    if (GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_IGNORE_PATHFINDING || !IsUsingPath())
...
bool Creature::ExecuteSpellEvent(Unit* target, SpellInfo const* spell) 
{ 
    if (!spell)
    {
        ExecuteVictim = NULL;
