	...
	if (flags & UPDATEFLAG_STATIONARY_POSITION)
	{
		...
		if (isType(TYPEMASK_GAMEOBJECT))
		{
			...
			case GAMEOBJECT_TYPE_FLAGDROP:
				...
				break;
			default:
                if (((GameObject*)this)->GetOwner()) // Implement offlike GO-summon animation.
                    updateType = UPDATETYPE_CREATE_OBJECT2;
                break;
			}
		}
...
bool WorldObject::CanSeeOrDetect(WorldObject const* obj, bool ignoreStealth, bool distanceCheck, bool checkAlert) const
{
    ...
    {
        if (thisGO->GetGoType() == GAMEOBJECT_TYPE_TRAP && GetTypeId() == TYPEID_PLAYER && thisGO->GetOwner() && thisGO->GetOwner()->GetTypeId() == TYPEID_PLAYER && thisGO->GetOwner()->getClass() == CLASS_HUNTER)
        {
            float DetectRange = 5.0f; // Default distance Melee attack dist. + Collision (1.5f)
            if (ToPlayer()->HasAura(58985))
                DetectRange += 3.0f; // Increase Detect Range from human racial
            if (ToPlayer()->HasAura(30894) || ToPlayer()->HasAura(30894))  
                DetectRange += 8.0f; // Increase Detect Range from rogue talents           
    ...
    if (const Player* target = obj->ToPlayer())
        if (const Player* me = ToPlayer())
            if ((target->HasAuraType(SPELL_AURA_MOD_STEALTH) || target->HasAuraType(SPELL_AURA_MOD_INVISIBILITY)) && me->IsSpectator())
                return false;
    ...
    if (const Player* player = ToPlayer())
        if (player->GetZoneId() == 215 && player->duel && player->duel->startTime && player->duel->opponent)
        {
            if (const Player* target = obj->ToPlayer())
                if (target->duel && player->duel->opponent == target)
                    return true;
            
            if (obj->ToGameObject() && ((obj->ToGameObject()->GetOwner() && (obj->ToGameObject()->GetOwner() == player->duel->opponent || obj->ToGameObject()->GetOwner() == player)) || !obj->ToGameObject()->GetOwner()))
                return true;
                
            if (obj->ToCreature() && obj->ToCreature()->GetOwner() && (obj->ToCreature()->GetOwner() == player->duel->opponent || obj->ToCreature()->GetOwner() == player))
                return true;

            if (obj->ToDynObject() && obj->ToDynObject()->GetCasterGUID() && (obj->ToDynObject()->GetCasterGUID() == player->duel->opponent->GetGUID() || obj->ToDynObject()->GetCasterGUID() == player->GetGUID()))
                return true;
...
bool WorldObject::CanDetectStealthOf(WorldObject const* obj, bool checkAlert) const
{
	...
	if (unit)
		combatReach = unit->GetCombatReach() + 0.5f;
    ...
    /// FIX ME: The biggest HackFix ever, please remake IT !!!
    if (unit && unit->GetTypeId() == TYPEID_PLAYER)
        if (const Unit* pUnit = obj->ToUnit())
            if (Aura* aura = pUnit->GetAura(26888))
                if (aura->GetDuration() > 9800)
                    return true;
