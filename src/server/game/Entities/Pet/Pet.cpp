...
bool Guardian::InitStatsForLevel(uint8 petlevel)
{
    ...
    switch (petType)
    {
        case SUMMON_PET:
        {
            ...
            if (GetOwner()->ToPlayer()) // Only player's pet has Base_attack set on lv, not creatures
            {
                SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, float((petlevel - (petlevel / 4))*0.5));
                SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, float((petlevel - (petlevel / 4))*0.5));
            }
        }
        case HUNTER_PET:
        {
        ...
        default:
        {
            switch (GetEntry())
            {
                case 510: // mage Water Elemental
                {
					...
					{
						// Convert Mage haste and hit into the mirror's haste/hit
						float bonus = GetOwner()->ToPlayer()->GetRatingBonusValue(CR_HASTE_SPELL);
                ...
                case 1964: //force of nature
                {
                    ...
                    float bonusDmg = GetOwner()->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_NATURE) * 0.1f;
                ...
                case 15352: //earth elemental 36213
                {
                    ...
                    float bonusDmg = GetOwner()->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_NATURE) * 0.6f;
                ...
                case 15438: //fire elemental
                {
                    ...
                    SetBonusDamage(int32(GetOwner()->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_FIRE) * 0.6f));
                ...
                case 29264: // Feral Spirit
                {
                   ...
                   float bonusDmg = 0;
                    if (m_owner)
                        bonusDmg = m_owner->GetTotalAttackPowerValue(BASE_ATTACK) * 0.07f;
                    if (m_owner && m_owner->HasAura(63271)) // Glyph
                        bonusDmg *= 1.30f;

                    SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, float((petlevel * 4 - petlevel) + (bonusDmg * 1.25f)));
                    SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, float((petlevel * 4 + petlevel) + bonusDmg));
				...
				case 31216: // Mirror Image
				{
                    ...
                    SetBonusDamage(int32(GetOwner()->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_FROST) * 0.21f));
                ...
                case 27829: // Ebon Gargoyle
                {
                    ...
                    if (Player *owner = m_owner->ToPlayer()) // get 100% of owning player's haste
                    {
                        ...
                        m_modSpellHitChance += m_owner->m_modSpellHitChance;
                        m_modMeleeHitChance += m_owner->m_modMeleeHitChance;
                        SetCreateHealth(uint32(owner->GetMaxHealth()*0.4)); // hp must be 55% + Create Health: ~20 000health (retail data) of DK hp
                    }
                                ...
                                case 49638:
                                    {
                                        if (SpellInfo const* proto = sSpellMgr->GetSpellInfo(itr->first))
                                            AddPct(Impurity, proto->Effects[0].CalcValue());
                                    }
                                    break;
                            }
                        }
