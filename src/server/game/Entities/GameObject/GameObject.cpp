            ...
            switch (GetGoType())
            {
                case GAMEOBJECT_TYPE_TRAP:
                {
                    ...
                    if (goInfo->trap.type == 2)
                        ...
                        m_cooldownTime = time(NULL) + 1;   // Hardcoded tooltip value ;
            ...
            if (isSpawned())
            {
                ...
                if (goInfo->type == GAMEOBJECT_TYPE_TRAP)
                {
                    ...
                    if (Unit* owner = GetOwner())
                    {
                        ...
                        if (!targets.empty())
                        {
                            targets.sort(Trinity::ObjectDistanceOrderPred(this));
                            target = targets.front();
                        }
                       /* std::list<Unit*> targets;
                        Trinity::AnyUnfriendlyNoTotemUnitInObjectRangeCheck checker(this, owner, radius);
                        Trinity::UnitListSearcher<Trinity::AnyUnfriendlyNoTotemUnitInObjectRangeCheck> searcher(this, targets, checker);
                        VisitNearbyWorldObject(radius, searcher);
                        for (std::list<Unit*>::const_iterator iter = targets.begin(); iter != targets.end(); ++iter)
                            if ((*iter) && owner->IsValidAttackTarget((*iter)))
                            {
                                if (!(*iter)->IsWithinDistInMap(this, radius))
                                    continue;
                                    
                                if (!(*iter)->IsWithinLOSInMap(this))
                                    continue;
...
void GameObject::Use(Unit* user)
{
    ...
    switch (GetGoType())
    {
        ...
        case GAMEOBJECT_TYPE_TRAP:                          //6
        {
            ...
            m_cooldownTime = time(NULL) + (goInfo->trap.cooldown ? goInfo->trap.cooldown :  uint32(2));   // template or 2 seconds
