...
class TC_GAME_API Unit : public WorldObject
{
    public:
        ...
        // player or player's pet resilience (-1%)
        uint32 GetMeleeCritDamageReduction(uint32 damage) const { return GetCombatRatingDamageReduction(CR_CRIT_TAKEN_MELEE, 2.6f, 34.3f, damage); }
        uint32 GetRangedCritDamageReduction(uint32 damage) const { return GetCombatRatingDamageReduction(CR_CRIT_TAKEN_RANGED, 2.7f, 34.3f, damage); }
        uint32 GetSpellCritDamageReduction(uint32 damage) const { return GetCombatRatingDamageReduction(CR_CRIT_TAKEN_SPELL, 2.7f, 34.3f, damage); }
        ...
        int32 GetMaxPositiveAuraModifierByMiscValue(AuraType auratype, int32 misc_value, bool debuffResist) const; // sacred cleansing hack fix https://github.com/TrinityCore/TrinityCore/issues/7406
		...
		uint32 ApplyCustomArenaBalance(Unit* victim, Unit* attacker, uint32 value);
