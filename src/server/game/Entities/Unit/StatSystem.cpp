...
#define ENTRY_EBON_GARGOYLE     27829
...
*/
    ...
    if (IsPet() && owner->getClass() == CLASS_WARLOCK)
        ToPet()->CastPetAuras(true);
...
void Guardian::UpdateMaxHealth()
{
    ...
    switch (GetEntry())
    {
        ...
        case ENTRY_EBON_GARGOYLE:   multiplicator = 1.9f;   break;
        case 31216:                 multiplicator = 6.0f;   break;
        case 24529:                 multiplicator = 5.0f;   break;
...
void Guardian::UpdateAttackPowerAndDamage(bool ranged)
{
    ...
    if (owner && owner->GetTypeId() == TYPEID_PLAYER)
    {
        if (IsHunterPet())                      //hunter pets benefit from owner's attack power
        {
            ...
            SetBonusDamage(int32(owner->GetTotalAttackPowerValue(RANGED_ATTACK) * 0.1487f * mod));
        }
        else if (IsPetGhoul() || IsRisenAlly()) //ghouls benefit from deathknight's attack power (may be summon pet or not)
        {
            bonusAP = owner->GetTotalAttackPowerValue(BASE_ATTACK) * 0.25f;
            SetBonusDamage(int32(owner->GetTotalAttackPowerValue(BASE_ATTACK) * 0.1487f));
        }
        else if (IsSpiritWolf()) //wolf benefit from shaman's attack power
        {
            float dmg_multiplier = 0.45f;
            if (m_owner->GetAuraEffect(63271, 0)) // Glyph of Feral Spirit
                dmg_multiplier = 0.75f;
        ...
        else if (GetEntry() == ENTRY_WATER_ELEMENTAL)
        {
            ...
            if (frost < 0)
                frost = 0;
            SetBonusDamage(int32(frost * 0.46f));
        }
        else if (GetEntry() == ENTRY_FELGUARD)
        {
            int32 shadow = int32(owner->GetUInt32Value(PLAYER_FIELD_MOD_DAMAGE_DONE_POS + SPELL_SCHOOL_SHADOW)) - owner->GetUInt32Value(PLAYER_FIELD_MOD_DAMAGE_DONE_NEG + SPELL_SCHOOL_SHADOW);
            if (shadow < 0)
                shadow = 0;
            SetBonusDamage(int32(shadow * 0.2f));
        }
    }
...
void Guardian::UpdateDamagePhysical(WeaponAttackType attType)
{
    ...
    // Custom Data - PEt get BonusDMG by 33% (need test)
