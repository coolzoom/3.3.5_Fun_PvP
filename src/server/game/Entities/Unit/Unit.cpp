...
#include "../../../scripts/Custom/custom_check.h"
...
void Unit::Update(uint32 p_time)
{
	...
	if (IsInCombat() && (GetTypeId() == TYPEID_PLAYER ||(ToCreature()->IsGuardian() || ToCreature()->IsPet())))
	{
    ...
    if (CustomRefreshTimmer > 600)
	{
		///FIXME: if you have CC auras in Stealt / Vanish your stealth must removed
        if (Unit* unit = ToUnit())
        {
            if (unit->HasUnitState(UNIT_STATE_STUNNED) || unit->HasUnitState(UNIT_STATE_CONFUSED) || unit->HasUnitState(UNIT_STATE_FLEEING))
                unit->RemoveAurasByType(SPELL_AURA_MOD_STEALTH);

            ///FIXME: Hackfix Set CombatState for Channeling Spell Drain Soul and Arcane missile
            if (unit->HasAura(47855) || unit->HasAura(42846) || unit->HasAura(605))
				unit->SetInCombatState(true, unit);
        }
		...
		if (Player* player = ToPlayer())
        {
			...
			if (Pet* pet = player->GetPet())
                if (pet->IsInCombat()) // Sharing combat with pet
                    if (pet->GetVictim())
                        player->CombatStart(pet->GetVictim(), true);
            // Guardian       
...
uint32 Unit::DealDamage(Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss)
{
    ...
    if ((damagetype == SPELL_DIRECT_DAMAGE || damagetype == DOT) && spellProto)
		sSpellDamage->Damage(damage, spellProto->Id);
    ...
    /// Hackfix for Stealth Removing when absorb (not all spells) PLEASE DELETE IT AND REPLACE WITH NORMAL FIX. TEMPOTARY HACKFIX    
	if (spellProto && victim && GetTypeId() == TYPEID_PLAYER && victim->GetTypeId() == TYPEID_PLAYER && victim->HasAuraType(SPELL_AURA_MOD_STEALTH) && (cleanDamage && damage <= 0) && !(spellProto->AttributesCu & SPELL_ATTR0_CU_DONT_BREAK_STEALTH) && (damagetype == DIRECT_DAMAGE || damagetype == SPELL_DIRECT_DAMAGE))
		victim->RemoveAurasByType(SPELL_AURA_MOD_STEALTH);
	if (!spellProto && victim && GetTypeId() == TYPEID_PLAYER && victim->GetTypeId() == TYPEID_PLAYER && victim->HasAuraType(SPELL_AURA_MOD_STEALTH) && (cleanDamage && damage <= 0) && (damagetype == DIRECT_DAMAGE || damagetype == SPELL_DIRECT_DAMAGE))
		victim->RemoveAurasByType(SPELL_AURA_MOD_STEALTH);
	...
	if (victim && victim->GetTypeId() == TYPEID_UNIT && victim->ToCreature()->IsTotem() && victim->ToTotem()->HasAura(8179))
		victim->ToTotem()->setDeathState(JUST_DIED);
...
SpellMissInfo Unit::MagicSpellHitResult(Unit* victim, SpellInfo const* spellInfo)
{
    ...
    // Cloak of Shadow
    if (!(spellInfo->SchoolMask & SPELL_SCHOOL_MASK_NORMAL) && !spellInfo->IsPositive() && spellInfo->SpellIconID != 3178)
    ...
    if (!spellInfo->IsPositive())
    {
        ...
        if (hasAura)
        {
            tmp += victim->GetMaxPositiveAuraModifierByMiscValue(SPELL_AURA_MOD_DEBUFF_RESISTANCE, int32(spellInfo->Dispel), false) * 100; // sacred cleansing hack fix https://github.com/TrinityCore/TrinityCore/issues/7406
...
Aura* Unit::_TryStackingOrRefreshingExistingAura(SpellInfo const* newAura, uint8 effMask, Unit* caster, int32* baseAmount /*= NULL*/, Item* castItem /*= NULL*/, ObjectGuid casterGUID /*= 0*/)
{
    ...
    if (!newAura->IsMultiSlotAura())
    {
        ...
        if (Aura* foundAura = GetOwnedAura(newAura->Id, casterGUID, newAura->HasAttribute(SPELL_ATTR0_CU_ENCHANT_PROC) ? castItemGUID : ObjectGuid::Empty, 0))
        {
            ...
            if (!(foundAura->GetEffectMask() & effMask)) // https://github.com/TrinityCore/TrinityCore/issues/9013
                return NULL;
			...
			&& (!caster || aura->GetCasterGUID() == caster))
		{
			if (Player* player = ToPlayer())
				if (player->HaveSpectators())
				{
					...
					msg.SetPlayer(player->GetName());
					if (const AuraApplication * aurApp = aura->GetApplicationOfTarget(caster))
						msg.RemoveAura(caster, spellId,
...
void Unit::RemoveAllAurasExceptType(AuraType type)
{
    ...
    {
        Aura const* aura = iter->second->GetBase();
        if (aura->GetSpellInfo()->HasAura(type) || aura->GetSpellInfo()->Id == 29659) // This 29659, Damage Bonus persist though death, for custom difficult in dungeons.
            ++iter;
        else
            _UnapplyAura(iter, AURA_REMOVE_BY_DEFAULT);
    }
    ...
    {
        Aura* aura = iter->second;
        if (aura->GetSpellInfo()->HasAura(type) || aura->GetSpellInfo()->Id == 29659)
            ++iter;
        else
            RemoveOwnedAura(iter, AURA_REMOVE_BY_DEFAULT);
    }
...
void Unit::RemoveAllAurasExceptType(AuraType type1, AuraType type2)
{
    ...
    {
        Aura const* aura = iter->second->GetBase();
        if (aura->GetSpellInfo()->HasAura(type1) || aura->GetSpellInfo()->HasAura(type2) || aura->GetSpellInfo()->Id == 29659)
            ++iter;
        else
            _UnapplyAura(iter, AURA_REMOVE_BY_DEFAULT);
    }
...
int32 Unit::GetMaxPositiveAuraModifierByMiscValue(AuraType auratype, int32 miscValue, bool debuffResist) const // sacred cleansing hack fix https://github.com/TrinityCore/TrinityCore/issues/7406
{
    int32 modifier = 0;
    ...
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
    {
        if ((*i)->GetMiscValue() == miscValue && (*i)->GetAmount() > modifier && (debuffResist || (!debuffResist && (*i)->GetId() != 53659))) // sacred cleansing hack fix https://github.com/TrinityCore/TrinityCore/issues/7406
            modifier = (*i)->GetAmount();
    }
...
{
    sSpellDamage->Damage(log->damage, log->SpellID);
...
void Unit::SendPeriodicAuraLog(SpellPeriodicAuraLogInfo* pInfo)
{
    ...
    sSpellDamage->Damage(pInfo->damage, aura->GetId());
        ...
        case SPELLFAMILY_WARRIOR:
        {
            ...
            if (dummySpell->SpellFamilyFlags[1] & 0x8)
            {
                ...
                if (!HasInArc(float(M_PI), victim))
                    return false;
                // Check if player is in stun and don't proc
    ...
    switch (dummySpell->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_MAGE:
        {
            ...
            switch (dummySpell->Id)
            {
                ...
                case 12848:
                {
                    // dont proc Ignite on Molten Armor crits. HACKFIX, need check SpellFamilyFlags instead SpellVisual
            ...
            // Molten Armor HACKFIX, need check SpellFamilyFlags instead SpellVisual
...
bool Unit::Attack(Unit* victim, bool meleeAttack)
{
	...
	//if (GetTypeId() == TYPEID_PLAYER && (HasAura(46924) || HasAura(51690)))
	//	return false;
        ...
        if (!HasFlag(UNIT_FIELD_AURASTATE, 1<<(flag-1)))
        {
            ...
            if (GetTypeId() == TYPEID_PLAYER)
            {
                ...
                for (PlayerSpellMap::const_iterator itr = sp_list.begin(); itr != sp_list.end(); ++itr)
                {
                    ...
                    {
                        if (itr->first == 44440 && ToPlayer()->HasAura(64357))
                            CastSpell(this, 44441, true, NULL);
                        else
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpell, uint32 damage, SpellInfo const* procAura)
{
    ...
    for (AuraApplicationMap::const_iterator itr = GetAppliedAuras().begin(); itr!= GetAppliedAuras().end(); ++itr)
    {
        ...
        // Hackfix for seal proc on absorb, Its required check Trigger effect in DBC (not implentemend yet)
		if (spellProto && (procExtra & PROC_EX_BLOCK || procExtra & PROC_EX_ABSORB))
		{
            ...
            case 31878: // Judgement of Wise (Rank 3)
            case 31877: // Judgement of Wise (Rank 2)
            case 31876: // Judgement of Wise (Rank 1)
            case 31930: // Judgement of Wise Proc
				active = true;
				break;
			default:
				break;
		...
		if ((procExtra & PROC_EX_ABSORB) && isVictim && (spellProto->Id == 33736 || spellProto->Id == 49284))
			continue;
...
void Unit::Kill(Unit* victim, bool durabilityLoss)
{
    ...
    if (creature)
    {
		...
		/*if (player && creature->getLevel() >= 83 && creature->GetCreatureTemplate()->Entry != 17256)
			if (creature->GetCreatureTemplate()->rank == CREATURE_ELITE_WORLDBOSS || creature->GetCreatureTemplate()->rank == CREATURE_ELITE_RARE)
				player->CreateRecentActivity(0, creature->GetCreatureTemplate()->Entry);*/
...
void Unit::ApplyResilience(Unit const* victim, float* crit, int32* damage, bool isCrit, CombatRating type) const
{
    ...
    case CR_CRIT_TAKEN_MELEE:
        if (source && damage)
        {
            ...
            {
                if (isCrit)
                    *damage -= target->GetMeleeCritDamageReduction(*damage)*0.81f;
                *damage -= target->GetMeleeDamageReduction(*damage)*1.62f;
                break;
            }
            if (source->getClass() == CLASS_PALADIN)
            {
                if (isCrit)
                    *damage -= target->GetMeleeCritDamageReduction(*damage)*0.7f;
                *damage -= target->GetMeleeDamageReduction(*damage)*1.4f;
                break;
            }
            ...
            {
                if (isCrit)
                    *damage -= target->GetMeleeCritDamageReduction(*damage)*0.78f;
                *damage -= target->GetMeleeDamageReduction(*damage)*1.56f;
                break;
            }
            else
            {
    ...
    case CR_CRIT_TAKEN_RANGED:
        if (source && damage)
        {
            ...
            {
                if (isCrit)
                    *damage -= target->GetRangedCritDamageReduction(*damage)*0.65f;
                *damage -= target->GetRangedDamageReduction(*damage)*1.3f;
                break;
            }
            if (source->getClass() == CLASS_ROGUE)
            {
    ...
    case CR_CRIT_TAKEN_SPELL:
        if (source && damage)
        {
            ...
            {
                if (isCrit)
                    *damage -= target->GetSpellCritDamageReduction(*damage)*0.7f;
                *damage -= target->GetSpellDamageReduction(*damage)*1.4f;
                break;
            }
            if (source->getClass() == CLASS_HUNTER)
            {
            ...
            {
                if (isCrit)
                    *damage -= target->GetSpellCritDamageReduction(*damage)*0.65f;
                *damage -= target->GetSpellDamageReduction(*damage)*1.3f;
                break;
            }
            if (source->getClass() == CLASS_DEATH_KNIGHT)
            {
                if (isCrit)
                    *damage -= target->GetSpellCritDamageReduction(*damage)*0.5f;
                *damage -= target->GetSpellDamageReduction(*damage)*1.0f;
                break;
            }
            if (source->getClass() == CLASS_SHAMAN)
            {
                if (isCrit)
                    *damage -= target->GetSpellCritDamageReduction(*damage)*0.6f;
                *damage -= target->GetSpellDamageReduction(*damage)*1.2f;
                break;
            }
            if (source->getClass() == CLASS_MAGE)
            {
