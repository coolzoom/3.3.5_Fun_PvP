...
void Item::SaveToDB(SQLTransaction& trans)
{
    ...
    switch (uState)
    {
        ...
        case ITEM_CHANGED:
        {
            ...
            stmt->setUInt16(++index, uint16(GetItemUpdateLevel()));
...
bool Item::LoadFromDB(ObjectGuid::LowType guid, ObjectGuid owner_guid, Field* fields, uint32 entry)
{
    ...
    //Load Ilv
    PreparedStatement*  stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_ITEM_LEVEL);
    stmt->setUInt32(0, guid);
    if (PreparedQueryResult result = CharacterDatabase.Query(stmt))
    {
        Field* fields = result->Fetch();
        m_itemUlevel = fields[0].GetUInt16();
    }
...
void Item::SetItemUpdateLevel(int32 level)
{
    if (level < 0)
        level = 0;
    ...
    if (Player* player = GetOwner())
    {
        ...
        SQLTransaction trans = CharacterDatabase.BeginTransaction();
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_ITEM_INSTANCE_LEVEL);
        stmt->setUInt16(0, uint16(level));
        stmt->setUInt32(1, uint32(GetGUID().GetRawValue()));
        trans->Append(stmt);
...
void Item::LoadItemQuery()
{
    if (Player* player = GetOwner())
    {
        if (ItemTemplate const* pProto = GetTemplate())
        {
            int32 level = GetItemUpdateLevel();
            std::ostringstream ost_name;
            std::string TrueName;
            uint32 diff = 0;
            ...
            if (level > 0)
            {
                if (pProto->ItemLevel > level)
                    diff = pProto->ItemLevel - level;
                else
                    diff = level - pProto->ItemLevel;
            }
            
            // DEBUG - TC_LOG_INFO("server.loading", "Loaded item guid: %u ; tempIlv: %u ; name: %s ; tmpilv: %u", uint32(GetGUID().GetRawValue()), level, pProto->Name1.c_str(), GetItemUpdateLevel());
            ...
            if (loc_idx >= 0)
                if (ItemLocale const* il = sObjectMgr->GetItemLocale(pProto->ItemId))
                {
                    ...
                    if (pProto->ItemLevel <= 284) // Custom: Do not load locale strings for custom items.
            ...
            data << ((level > 0) ? level : pProto->ItemLevel);
                ...
                {   
                    ...
                    if (level <= 284)
                    {
                        switch (pProto->ItemStat[i].ItemStatType)
                        {
                            ...
                            case 42:
                            case 45:
                                value = (pProto->ItemStat[i].ItemStatValue * pow(sp, diff));
                                if (pProto->ItemLevel > level)
                                    value = (pProto->ItemStat[i].ItemStatValue / pow(sp, diff));
                                break;
                            case 3: // Stats
                            ...
                            case 6:
                            case 7:
                                value = (pProto->ItemStat[i].ItemStatValue * pow(stats, diff));
                                if (pProto->ItemLevel > level)
                                    value = (pProto->ItemStat[i].ItemStatValue / pow(stats, diff));
                                break;
                            default: 
                                value = (pProto->ItemStat[i].ItemStatValue * pow(rat, diff));
                                if (pProto->ItemLevel > level)
                                    value = (pProto->ItemStat[i].ItemStatValue / pow(rat, diff));
                                break;
                        }
                    ...
                    else if (level > 284)
                    {
                        switch (pProto->ItemStat[i].ItemStatType)
                        {
                            ...
                            case 42:
                            case 45:
                                value = (pProto->ItemStat[i].ItemStatValue * pow(sp_H, diff));
                                if (pProto->ItemLevel > level)
                                    value = (pProto->ItemStat[i].ItemStatValue / pow(sp_H, diff));
                                break;
                            case 3: // Stats
                            ...
                            case 6:
                            case 7:
                                value = (pProto->ItemStat[i].ItemStatValue * pow(stats_H, diff));
                                if (pProto->ItemLevel > level)
                                    value = (pProto->ItemStat[i].ItemStatValue / pow(stats_H, diff));
                                break;
                            default: 
                                value = (pProto->ItemStat[i].ItemStatValue * pow(rat_H, diff));
                                if (pProto->ItemLevel > level)
                                    value = (pProto->ItemStat[i].ItemStatValue / pow(rat_H, diff));
                                break;
                        }
            ...
            for (int i = 0; i < MAX_ITEM_PROTO_DAMAGES; ++i)
            {
                if (level > 0)
                {
                    ...
                    if (level <= 284)
                    {
                        value = (pProto->Damage[i].DamageMin * pow(dmg, diff));
                        if (pProto->ItemLevel > level)
                            value = (pProto->Damage[i].DamageMin / pow(dmg, diff));
                            
                        value1 = (pProto->Damage[i].DamageMax * pow(dmg, diff));
                        if (pProto->ItemLevel > level)
                            value1 = (pProto->Damage[i].DamageMax / pow(dmg, diff));
                    }
                    else
                    {
                        value = (pProto->Damage[i].DamageMin * pow(dmg_H, diff));
                        if (pProto->ItemLevel > level)
                            value = (pProto->Damage[i].DamageMin / pow(dmg_H, diff));
                            
                        value1 = (pProto->Damage[i].DamageMax * pow(dmg_H, diff));
                        if (pProto->ItemLevel > level)
                            value1 = (pProto->Damage[i].DamageMax / pow(dmg_H, diff));
                    }
            ...
            if (level > 0)
            {
                float value = (pProto->Armor * pow(armor, diff));
                if (pProto->ItemLevel > level)
                    value = (pProto->Armor / pow(armor, diff));
                data << int32(value);;
            }
            else
                data << pProto->Armor;
            ...
            if (level > 0)
                data << pProto->Block * pow(1.0024f, diff);
            else
                data << pProto->Block;
            data << pProto->ItemSet;
