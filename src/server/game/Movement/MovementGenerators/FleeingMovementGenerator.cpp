...
void FleeingMovementGenerator<T>::_setTargetLocation(T* owner)
{
    ...
    i_nextCheckTime.Reset((traveltime / 2) + urand(800, 1500));
}
...
void FleeingMovementGenerator<T>::_getPoint(T* owner, float &x, float &y, float &z)
{
    ...
    if (float(owner->GetPositionZ() - pos.m_positionZ) > 2.5f)
    {
        Position post = owner->GetFirstCollisionPosition(1.0f, angle);
        x = post.m_positionX;
        y = post.m_positionY;
        z = post.m_positionZ;
    }
...
void FleeingMovementGenerator<T>::DoInitialize(T* owner)
{
    ...
    if (owner->HasUnitMovementFlag(MOVEMENTFLAG_HOVER))
    {
        owner->StopMoving();
        i_nextCheckTime.Reset(200);
        return;
