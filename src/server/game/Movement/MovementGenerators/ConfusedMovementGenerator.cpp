...
{	
    ...
    float const wander_distance = 3;
    float x = unit->GetPositionX();
    float y = unit->GetPositionY();
    float z = unit->GetPositionZ();
    StartX = unit->GetPositionX();
    StartY = unit->GetPositionY();
    StartZ = unit->GetPositionZ();
...
bool ConfusedMovementGenerator<T>::DoUpdate(T* unit, uint32 diff)
{
    ...
    {
        UpdateDist = 1000;
        if (unit->GetDistance(StartX, StartY, StartZ) > 5.0f)
        {
            unit->StopMoving();
            i_nextMove = urand(1, MAX_CONF_WAYPOINTS);
            i_nextMoveTime.Reset(urand(1000, 2000)); // Guessed
            return false;
        }
    }
    else
    ...
    if (i_nextMoveTime.Passed())
    {
        ...
        {
            i_nextMove = urand(1, MAX_CONF_WAYPOINTS);
            i_nextMoveTime.Reset(urand(100, 200)); // Guessed
        }
    }
    else
    {
