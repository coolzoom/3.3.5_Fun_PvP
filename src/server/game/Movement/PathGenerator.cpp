...
bool PathGenerator::CalculatePath(float destX, float destY, float destZ, bool forceDest, bool straightLine)
{
	...
	if (!_navMesh || !_navMeshQuery || _sourceUnit->HasUnitState(UNIT_STATE_IGNORE_PATHFINDING) || (_sourceUnit->ToCreature() && !_sourceUnit->ToCreature()->IsUsingPath()) ||
...
void PathGenerator::BuildPointPath(const float *startPoint, const float *endPoint)
{
	...
	if (_sourceUnit->GetMapId() == 562)
	{
		...
		if (endPoint[1] > 8.0f && endPoint[2] <= 6257.553223f && endPoint[2] >= 6241.040430f && endPoint[0] >= 264.060000f && endPoint[0] <= 281.508940f)      // #1 pillar
		{
			if (startPoint[1] <= 8.0f && _sourceUnit->GetTypeId() == TYPEID_PLAYER)
			{
				_type = PATHFIND_NOPATH;
				return;
			}

			if (startPoint[1] <= 8.0f && (_sourceUnit->IsPet() || _sourceUnit->IsGuardian()))
			{
				Clear();
				_pathPoints.resize(pointCount + 4);
				_pathPoints[pointCount + 0] = G3D::Vector3(startPoint[2], startPoint[0], startPoint[1]);
				_pathPoints[pointCount + 1] = G3D::Vector3(6241.496446f, 265.674106f, 11.500000f);
				_pathPoints[pointCount + 2] = G3D::Vector3(6246.895469f, 271.965563f, 11.500000f);
				_pathPoints[pointCount + 3] = G3D::Vector3(endPoint[2], endPoint[0], endPoint[1]);

			}

			if (startEndDist > 8.0f || ((_sourceUnit->IsPet() || _sourceUnit->IsGuardian()) && startEndDist > 12.0f))
			{
				Clear();
				pointCount = 4;
				_pathPoints.resize(pointCount);
				_pathPoints[0] = G3D::Vector3(startPoint[2], startPoint[0], startPoint[1]);
				_pathPoints[1] = G3D::Vector3(6241.496446f, 265.674106f, 11.500000f);
				_pathPoints[2] = G3D::Vector3(6246.895469f, 271.965563f, 11.500000f);
				_pathPoints[3] = G3D::Vector3(endPoint[2], endPoint[0], endPoint[1]);
		...
		if (startPoint[1] > 8.0f && startPoint[2] <= 6257.553223f && startPoint[2] >= 6241.040430f && startPoint[0] >= 264.060000f && startPoint[0] <= 281.598940f)       // #1 pillar
		{
			if (endPoint[1] <= 9.0f && _sourceUnit->GetTypeId() == TYPEID_PLAYER)
			{
				_type = PATHFIND_NOPATH;
				return;
			}

			if (endPoint[1] <= 9.0f && (_sourceUnit->IsPet() || _sourceUnit->IsGuardian()))
			{
				Clear();
				_pathPoints.resize(pointCount + 4);
				_pathPoints[pointCount + 0] = G3D::Vector3(startPoint[2], startPoint[0], startPoint[1]);
				_pathPoints[pointCount + 1] = G3D::Vector3(6246.895469f, 271.965563f, 11.500000f);
				_pathPoints[pointCount + 2] = G3D::Vector3(6241.496446f, 265.674106f, 11.500000f);
				_pathPoints[pointCount + 3] = G3D::Vector3(endPoint[2], endPoint[0], endPoint[1]);
			...
			if (startEndDist > 8.0f || ((_sourceUnit->IsPet() || _sourceUnit->IsGuardian()) && startEndDist > 12.0f))
			{
				...
				_pathPoints.resize(pointCount);
				_pathPoints[0] = G3D::Vector3(startPoint[2], startPoint[0], startPoint[1]);
				_pathPoints[1] = G3D::Vector3(6246.895469f, 271.965563f, 11.500000f);
				_pathPoints[2] = G3D::Vector3(6241.496446f, 265.674106f, 11.500000f);
				_pathPoints[3] = G3D::Vector3(endPoint[2], endPoint[0], endPoint[1]);
			}
			else
			{
		...
		if (startPoint[1] > 8.0f && startPoint[2] >= 6220.645660f && startPoint[2] <= 6238.851660f && startPoint[0] >= 241.305917f && startPoint[0] <= 258.788794f) // #2 pillar
		{
			if (endPoint[1] <= 9.0f && _sourceUnit->GetTypeId() == TYPEID_PLAYER)
			{
				_type = PATHFIND_NOPATH;
			...
			if (endPoint[1] <= 9.0f && (_sourceUnit->IsPet() || _sourceUnit->IsGuardian()))
			{
				...
				_pathPoints.resize(pointCount + 4);
				_pathPoints[pointCount + 0] = G3D::Vector3(startPoint[2], startPoint[0], startPoint[1]);
				_pathPoints[pointCount + 1] = G3D::Vector3(6230.592484f, 252.190030f, 11.500000f);
				_pathPoints[pointCount + 2] = G3D::Vector3(6235.734219f, 258.080000f, 11.500000f);
				_pathPoints[pointCount + 3] = G3D::Vector3(endPoint[2], endPoint[0], endPoint[1]);
			}
			...
			if (startEndDist > 8.0f || ((_sourceUnit->IsPet() || _sourceUnit->IsGuardian()) && startEndDist > 12.0f))
			{
				...
				_pathPoints.resize(pointCount);
				_pathPoints[0] = G3D::Vector3(startPoint[2], startPoint[0], startPoint[1]);
				_pathPoints[1] = G3D::Vector3(6230.592484f, 252.190030f, 11.500000f);
				_pathPoints[2] = G3D::Vector3(6235.734219f, 258.080000f, 11.500000f);
				_pathPoints[3] = G3D::Vector3(endPoint[2], endPoint[0], endPoint[1]);
			}
			else
			{
		...
		if (endPoint[1] > 8.0f && endPoint[2] >= 6220.645660f && endPoint[2] <= 6237.851660f && endPoint[0] >= 241.305917f && endPoint[0] <= 258.788794f) // #2 pillar
		{
			if (startPoint[1] <= 8.0f && _sourceUnit->GetTypeId() == TYPEID_PLAYER)
			{
				_type = PATHFIND_NOPATH;
			...
			if (startPoint[1] <= 8.0f && (_sourceUnit->IsPet() || _sourceUnit->IsGuardian()))
			{
				...
				_pathPoints.resize(pointCount + 4);
				_pathPoints[pointCount + 0] = G3D::Vector3(startPoint[2], startPoint[0], startPoint[1]);
				_pathPoints[pointCount + 1] = G3D::Vector3(6235.734219f, 258.080000f, 11.500000f);
				_pathPoints[pointCount + 2] = G3D::Vector3(6230.592484f, 252.190030f, 11.500000f);
				_pathPoints[pointCount + 3] = G3D::Vector3(endPoint[2], endPoint[0], endPoint[1]);
			...
			if (startEndDist > 8.0f || ((_sourceUnit->IsPet() || _sourceUnit->IsGuardian()) && startEndDist > 12.0f))
			{
				...
				_pathPoints.resize(pointCount);
				_pathPoints[0] = G3D::Vector3(startPoint[2], startPoint[0], startPoint[1]);
				_pathPoints[1] = G3D::Vector3(6235.734219f, 258.080000f, 11.500000f);
				_pathPoints[2] = G3D::Vector3(6230.592484f, 252.190030f, 11.500000f);
				_pathPoints[3] = G3D::Vector3(endPoint[2], endPoint[0], endPoint[1]);
			}
			else
			{
