...
, m_spellValue(new SpellValue(m_spellInfo)), m_preGeneratedPath(PathGenerator(m_caster))
{
    ...
    // Arcane Missiles 
    if (m_spellInfo->SpellFamilyName == SPELLFAMILY_MAGE && (m_spellInfo->Id == 42845 || m_spellInfo->Id == 38264))
        m_canReflect = true;
        
    // Peneance
    if (m_spellInfo->SpellFamilyName == SPELLFAMILY_PRIEST && m_spellInfo->Id == 53000)
        m_canReflect = true;
        
    // Magic Dispel & Grip
    if ((m_spellInfo->Id == 30449 || m_spellInfo->Id == 988)/* || (m_spellInfo->SpellFamilyName == SPELLFAMILY_DEATHKNIGHT && m_spellInfo->SpellFamilyFlags[0] == 0x2000000 && m_spellInfo->Effects[0].Mechanic == MECHANIC_GRIP)*/)
        m_canReflect = true;
...
void Spell::SelectExplicitTargets()
{
	...
	if (Unit* target = m_targets.GetUnitTarget())
	{
		...
		switch (m_spellInfo->Id)
		{
			...
			case 30449:    // Spell Steal
			case 51514:    // Hex
            case 8012:     // Purge
            case 48300:    // Devouring Plague
				if (Aura* aura = target->GetAura(8178))
					if (Unit* grounding = aura->GetOwner()->ToUnit())
					{
						// Redirect target
						m_targets.SetUnitTarget(grounding);
						grounding->setDeathState(JUST_DIED);
					}
				break;
		}
    ...
    switch (targetType.GetTarget())
    {
        ...
        default:
        {
            ...
            switch (targetType.GetTarget())
            {
                ...
                case TARGET_DEST_CASTER_BACK_RIGHT:
                case TARGET_DEST_CASTER_FRONT_RIGHT:
                    dist += 0.7f * 2.5f; // Increasing distance meter 7/10 * 2.5
...
void Spell::prepareDataForTriggerSystem(AuraEffect const* /*triggeredByAura*/)
{
    ...
    if (m_spellInfo->IsTrapSpellEffect())
...
void Spell::AddUnitTarget(Unit* target, uint32 effectMask, bool checkIfValid /*= true*/, bool implicit /*= true*/, Position const* losPosition /*= nullptr*/)
{
	...
	else if (GetCCDelay(m_spellInfo))
	{
		targetInfo.timeDelay = GetCCDelay(m_spellInfo);
		if (m_delayMoment == 0 || m_delayMoment > targetInfo.timeDelay)
			m_delayMoment = targetInfo.timeDelay;
...
void Spell::DoAllEffectOnTarget(TargetInfo* target)
{
    ...
    if (target->processed)
    {
        if (Unit* pUnit = ObjectAccessor::GetUnit(*m_caster, target->targetGUID)) 
            if (pUnit->ToPlayer() && m_spellInfo->HasAttribute(SPELL_ATTR0_CU_CANT_FADED)) // TempUnit remove stealth
                pUnit->RemoveAurasByType(SPELL_AURA_MOD_STEALTH);
        return;
    }
    ...
    if (!unit)
    {
        ...
        if (unit->ToPlayer() && m_spellInfo->HasAttribute(SPELL_ATTR0_CU_CANT_FADED)) // TempUnit remove stealth
   ...
   VisibleTimeDelayed = false;

    if (unit && unit->ToPlayer())
        if (Aura* aura = unit->GetAura(26888))
            if (uint32(aura->GetDuration()) < 9760)
                VisibleTimeDelayed = true;
    
    if (m_spellInfo->Id == 51724 || m_spellInfo->Id == 64695)
        VisibleTimeDelayed = true;
        
    if ((m_spellInfo->Id == 48158 || m_spellInfo->Id == 605) && missInfo != SPELL_MISS_NONE)
        return; // SWD HACKFIX - Should not trigger self dmg if miss/resist etc
        
    if (m_spellInfo->HasAttribute(SPELL_ATTR0_CU_CANT_FADED) && missInfo == SPELL_MISS_NONE) // TempUnit remove stealth
        unit->RemoveAurasByType(SPELL_AURA_MOD_STEALTH);

	if ((getState() == SPELL_STATE_DELAYED || m_spellInfo->Id == 605) && !m_spellInfo->IsPositive() && (getMSTime() - target->timeDelay) <= unit->m_lastSanctuaryTime && !caster->HasAura(34709)) // can't evaded if you are visible
	{
		if (m_spellInfo->HasAttribute(SPELL_ATTR0_CU_CANT_FADED) && missInfo == SPELL_MISS_NONE) // Can't break vanish when missed
		{
			unit->RemoveAurasByType(SPELL_AURA_MOD_STEALTH);
			// Conditions for vanishing
			if (!VisibleTimeDelayed) // Sap Can't vanished
				return;
		}
        if (!m_spellInfo->HasAttribute(SPELL_ATTR0_CU_CANT_FADED))
			return;
	}   // No missinfo in that case                                         // No missinfo in that case
    ...
    if (canEffectTrigger && !procAttacker && !procVictim)
    {
        ...
        if (missInfo == SPELL_MISS_REFLECT)
            positive = false; // Can't reflect positive spell, also positive spell should convert to negative like penance or magic dispel
...
SpellMissInfo Spell::DoSpellHitOnUnit(Unit* unit, uint32 effectMask, bool scaleAura)
{
	...
	if ((m_spellInfo->Speed || GetCCDelay(m_spellInfo)) && (unit->IsImmunedToDamage(m_spellInfo) || unit->IsImmunedToSpell(m_spellInfo)))
		return SPELL_MISS_IMMUNE;

	if (m_spellInfo->Id == 33786 && (unit->IsImmunedToDamage(m_spellInfo) || unit->HasAura(33786)))
		return SPELL_MISS_IMMUNE;
    ...
    for (uint32 effectNumber = 0; effectNumber < MAX_SPELL_EFFECTS; ++effectNumber)
    {
        if (effectMask & (1 << effectNumber))
        {
            ...
            {
                int32 debuff_resist_chance = unit->GetMaxPositiveAuraModifierByMiscValue(SPELL_AURA_MOD_DEBUFF_RESISTANCE, int32(m_spellInfo->Dispel), true); // sacred cleansing hack fix https://github.com/TrinityCore/TrinityCore/issues/7406
	...
	if (m_spellInfo->HasAttribute(SPELL_ATTR0_CU_CANT_FADED) && returnVal != SPELL_MISS_RESIST) // Can't break vanish when missed
		unit->RemoveAurasByType(SPELL_AURA_MOD_STEALTH);
    ...
    if (m_caster != unit)
    {
		...
		if ((m_spellInfo->Speed > 0.0f || GetCCDelay(m_spellInfo)) && unit->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE) && unit->GetCharmerOrOwnerGUID() != m_caster->GetGUID() && !m_caster->HasAura(605))
			return SPELL_MISS_EVADE;
        ...
        {			
			if(unit->HasAura(6215))
			{
				...
				if(unit->HasAura(6215) && unit->GetAura(6215)->GetDuration() > tiempo*0.5)
					...
					{
						//char* msg = "A more powerful spell is already active";
        ...
        if (m_caster->_IsValidAttackTarget(unit, m_spellInfo))
        {
			...
			// Binary Resistance Include Creatures and pets
			/*if (!(m_spellInfo->SchoolMask & SPELL_SCHOOL_MASK_NORMAL) && !(m_spellInfo->SchoolMask & SPELL_SCHOOL_MASK_HOLY) && !m_spellInfo->IsPositive() && !m_caster->IsTotem() && m_caster->GetTypeId() != TYPEID_GAMEOBJECT && m_spellInfo->SpellIconID != 3178)
			{
				int32 resistChance = unit->GetResistance(SpellSchoolMask(m_spellInfo->SchoolMask));
				int32 SpellPenetration = 0;
				if (resistChance > 0 && m_spellInfo->AttributesCu & SPELL_ATTR0_CU_CAN_RESIST)
				{
					if ((m_caster->IsPet() || m_caster->IsGuardian()) && m_caster->GetOwner()->GetTypeId() == TYPEID_PLAYER)
						SpellPenetration += m_caster->GetOwner()->GetSpellPenetration(SpellSchoolMask(m_spellInfo->SchoolMask));

					if (m_caster->GetTypeId() == TYPEID_PLAYER) // Creatures dont have Spell Penetration
						SpellPenetration += m_caster->GetSpellPenetration(SpellSchoolMask(m_spellInfo->SchoolMask));

					// Check if Creature
					if (m_caster->GetTypeId() != TYPEID_PLAYER)
						//Boss Factor
						if (m_caster->ToCreature()->getLevel() > 80)
							SpellPenetration += (m_caster->ToCreature()->getLevel() - 80) * 75;

					if (SpellPenetration && SpellPenetration < resistChance)
						resistChance = resistChance - SpellPenetration;
					else if (resistChance < 0 || SpellPenetration > resistChance)
						resistChance = 0;

					resistChance *= 1.2f; // Resist Chance Factor

					if (resistChance > 0 && resistChance > irand(0, 1000))
        ...
        else if (m_caster->IsFriendlyTo(unit))
        {
			...
			if ((m_spellInfo->Speed > 0.0f || GetCCDelay(m_spellInfo)) && unit->GetTypeId() == TYPEID_PLAYER && !m_spellInfo->IsPositive() && m_spellInfo->Id != 63881 && unit->GetCharmerOrOwnerGUID() != m_caster->GetGUID() && !m_caster->HasAura(605))
				return SPELL_MISS_EVADE;
			...
			{
                for (uint32 i = 0; i < MAX_SPELL_EFFECTS; ++i)
                    if (!m_spellInfo->IsPassive() && !m_spellInfo->Effects[i].IsAreaAuraEffect() && !m_spellInfo->Effects[i].IsEffect(SPELL_EFFECT_PERSISTENT_AREA_AURA))
                        if (unit->GetMap()->IsDungeon())
                        {
                            m_caster->SetInCombatState(false, unit);
                            if (!unit->getHostileRefManager().isEmpty() && !unit->IsPet())
...
void Spell::prepare(SpellCastTargets const* targets, AuraEffect const* triggeredByAura)
{
    ...
    if (DisableMgr::IsDisabledFor(DISABLE_TYPE_SPELL, m_spellInfo->Id, m_caster) ||
        (m_spellInfo->Id == 71726 && m_caster->GetAreaId() != 4891)) // Hackfix for Vampiric Bite being used on other bosses.
    {
...
void Spell::cast(bool skipCheck)
{
	...
	if (((m_spellInfo->Speed > 0.0f || GetCCDelay(m_spellInfo)) && !m_spellInfo->IsChanneled()) || m_spellInfo->Id == 14157)
	{
...
void Spell::SendChannelStart(uint32 duration)
{
    ...
    if (m_spellInfo->Id == 605 && channelTarget)
        if (Unit* selectedUnit = ObjectAccessor::GetUnit(*m_caster, channelTarget))
            if (selectedUnit->GetTypeId() == TYPEID_PLAYER)
                duration /= 6;
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (!m_spellInfo->IsPassive())
    {
        ...
        if (!m_caster->GetSpellHistory()->IsReady(m_spellInfo, m_castItemEntry, IsIgnoringCooldowns()))
        {
                ...
                else
                {
	            	...
	            	return (m_spellInfo->Id == 15473 || m_spellInfo->Id == 46584) ? SPELL_FAILED_FIZZLE : SPELL_FAILED_NOT_READY;
                }
        }
    }
    ...
    if (Unit* target = m_targets.GetUnitTarget())
    {
        ...
        if (target != m_caster)
        {
            ...
            if (m_caster->GetEntry() != WORLD_TRIGGER) // Ignore LOS for gameobjects casts (wrongly cast by a trigger)
            {
				...
				if (!m_spellInfo->HasAttribute(SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS) && !DisableMgr::IsDisabledFor(DISABLE_TYPE_SPELL, m_spellInfo->Id, NULL, SPELL_DISABLE_LOS) && !target->IsWithinLOSInMap(losTarget) && !m_caster->IsPet())
					return SPELL_FAILED_LINE_OF_SIGHT;
                    
                if (!m_spellInfo->HasAttribute(SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS) && !DisableMgr::IsDisabledFor(DISABLE_TYPE_SPELL, m_spellInfo->Id, NULL, SPELL_DISABLE_LOS) && !target->IsWithinLOSInMap(losTarget) && m_caster->IsPet())
                {
                    if (m_caster->ToPet() && m_caster->ToPet()->AI())
                    {
                        m_caster->ToPet()->AI()->AttackStart(target); // TempFix for casting spells with pet if target is behind texture
                        m_caster->ToPet()->ExecuteSpellEvent(target, m_spellInfo);
                    }
                    return SPELL_FAILED_LINE_OF_SIGHT;
                }
            }
    ...
    if (m_caster->HasAura(12292) && (m_spellInfo->SpellIconID == 95)) // Enrage effect can't remove Death wish hackfix
        return SPELL_FAILED_DONT_REPORT;
        ...
        switch (m_spellInfo->Effects[i].Effect)
        {
            ...
            case SPELL_EFFECT_CHARGE:
            {
                ...
                if (GetSpellInfo()->NeedsExplicitUnitTarget())
                {
                    ...
                    float objSize = target->GetObjectSize();
                    float range = m_spellInfo->GetMaxRange(true, m_caster, this) * 1.75f + objSize; // can't be overly strict
                    // Hackfixes -- Fix problem without offmesh
                    if (GetSpellInfo()->Id == 54216 || (m_caster->GetMapId() == 562 && (m_caster->GetPositionZ() > 8.0f && target->GetPositionZ() > 8.0f))) // Master's call Charging Pet Range wrong
                    {
                        range = m_spellInfo->GetMaxRange(true, m_caster, this) * 60.0f + objSize; // can't be overly strict
                        m_preGeneratedPath.SetPathLengthLimit(range);
                        
                        // first try with raycast, if it fails fall back to normal path
                        bool result = m_preGeneratedPath.CalculatePath(target->GetPositionX(), target->GetPositionY(), target->GetPositionZ() + target->GetObjectSize(), false, false);
            ...
            case SPELL_EFFECT_TALENT_SPEC_SELECT:
                ...
                if (m_caster->GetTypeId() == TYPEID_PLAYER)
                {
                    ...
                    if (plr->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_2v2_SOLO) ||
                        plr->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_5v5))
                    {
                        plr->GetSession()->SendNotification("ERROR: You can't change your talents while in queue for 1v1 or 3v3 Solo.");
                        return SPELL_FAILED_DONT_REPORT;
                    }
                    if (Battleground const* bg = m_caster->ToPlayer()->GetBattleground())
                        if (bg->GetStatus() == STATUS_IN_PROGRESS)
                            return SPELL_FAILED_NOT_IN_BATTLEGROUND;
                        // Fix bug With DualSpec and 100% crit chance.
                        if (m_caster->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_DISARMED))
                            return SPELL_FAILED_DONT_REPORT;
                }
                break;
            default:
...
uint32 Spell::GetCCDelay(SpellInfo const* _spell)
{
	...
	if (_spell->Id == 34709) // Sight eye (Arena Buff) has no delay time
		return 0;
    if (_spell->Id == 49575) // Death Grip - Trigger has no delay time
        return 0;
    if (_spell->Id == 44032)
        return 0;
    if (_spell->Id == 6358)
	...
	case SPELLFAMILY_HUNTER:
       ...
       if (_spell->SpellIconID == 20)
            return 0;
        if (_spell->Id == 34490) // Silencing Shot
	...
	case SPELLFAMILY_PRIEST:
		...
		if (_spell->Id == 10890)
			return delayForInstantSpells;
        // Devouring Plague proc
        if (_spell->Id == 48300)
            return 0; // This dealy is not correct
        if (_spell->Id == 605) // Mind Control
            return 200;
        break;
	...
	case SPELLFAMILY_MAGE:
		// Burning Determ. Fixed Issue with permaimmunity
		if (_spell->Id == 54748)
			return 300;
		// Deep Freeze
		if (_spell->Id == 44572)
	...
	case SPELLFAMILY_WARRIOR:
		...
		if (_spell->Id == 18498 || _spell->Id == 74347 || _spell->Id == 57755)
			return 0;
		break;
	case SPELLFAMILY_WARLOCK:
		//Spell Lock - Debuff
		if (_spell->Id == 24259)
			return 130; // Offlike Delay
        if (_spell->Id ==  27223)
            return 200; // Coil have travel time.
		break;
	...
	if ((_spell->Dispel == DISPEL_CURSE) && !_spell->IsPositive())
		return delayForInstantSpells; // Curses have delay
...
SpellCastResult Spell::CheckCasterAuras() const
{
    ...
    bool usableInCC = (m_spellInfo->HasAttribute(SPELL_ATTR5_USABLE_WHILE_CONFUSED) && m_spellInfo->HasAttribute(SPELL_ATTR5_USABLE_WHILE_FEARED));
	// Glyph of Pain Suppression
	// there is no other way to handle it
	if (m_spellInfo->Id == 33206 && m_caster->HasAura(63248))
		usableInStun = true;
	else if (m_spellInfo->Id == 33206)
		usableInStun = false;
        
    // This is hackfix -> Make it in spellMgr to remove flags (usable in) Hand of Protection
    ...
    if (unitflag & UNIT_FLAG_STUNNED)
    {
        ...
        if (usableInStun)
        {
            ...
            if (foundNotStun && m_spellInfo->Id != 22812 && m_spellInfo->Id != 1044)
                prevented_reason = SPELL_FAILED_STUNNED;
        }
        else
            prevented_reason = SPELL_FAILED_STUNNED;
    }
    else if (unitflag & UNIT_FLAG_CONFUSED && (!m_spellInfo->HasAttribute(SPELL_ATTR5_USABLE_WHILE_CONFUSED) && !usableInCC))
        prevented_reason = SPELL_FAILED_CONFUSED;
    else if (unitflag & UNIT_FLAG_FLEEING && (!m_spellInfo->HasAttribute(SPELL_ATTR5_USABLE_WHILE_FEARED) && !usableInCC))
...
bool Spell::CheckEffectTarget(Unit const* target, uint32 eff, Position const* losPosition) const
{
	...
	if (target->HasAura(8178)) // Ignore LOS for gameobjects casts (wrong casted by a trigger)
		return true;
    ...
    switch (m_spellInfo->Effects[eff].Effect)
    {
        ...
        default:                                            // normal case
        {
            // This is tempotary, you could not start cast something (or suc. cast) if target is not in los, casted effect should ignore los. this will fix bugs with LOS. need test
            if (losPosition)
...
{
	return m_spellInfo->SpellVisual[0] || m_spellInfo->SpellVisual[1] || m_spellInfo->IsChanneled() || (GetCCDelay(m_spellInfo) > 0 ||
