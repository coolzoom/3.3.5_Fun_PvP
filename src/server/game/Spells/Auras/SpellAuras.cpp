...
void Aura::HandleAuraSpecificMods(AuraApplication const* aurApp, Unit* caster, bool apply, bool onReapply)
{
    ...
    if (apply)
    {
        if (m_spellInfo->Id == 6358) // seduction
                if (caster->GetOwner()->HasAura(56250) && target) // Glyph of Seduction
                {
        ...
        // @todo while invisible buff get Stun / Fear ... Invisible buff should removed 
		if ((m_spellInfo->AttributesCu & SPELL_ATTR0_CU_AURA_CC) && target->IsControlledByPlayer())
			target->RemoveAurasDueToSpell(66);
            
        ///@todo Remove Grounding totem on Polymorph & fear.. HACKFIX
        if (target->GetTypeId() == TYPEID_UNIT && target->IsTotem() && target->HasAura(8179))
            if (m_spellInfo->Dispel == DISPEL_CURSE || m_spellInfo->HasAura(SPELL_AURA_MOD_DECREASE_SPEED) ||
                m_spellInfo->HasAura(SPELL_AURA_MOD_STUN) || m_spellInfo->HasAura(SPELL_AURA_MOD_ROOT) || m_spellInfo->HasAura(SPELL_AURA_MOD_CONFUSE) ||
                m_spellInfo->HasAura(SPELL_AURA_MOD_FEAR) || m_spellInfo->HasAura(SPELL_AURA_MOD_SILENCE) ||  m_spellInfo->HasAura(SPELL_AURA_PERIODIC_DAMAGE) ||
                m_spellInfo->Id == 49560 || m_spellInfo->Id == 47865 || m_spellInfo->Id == 18223 || m_spellInfo->Id == 11719 || m_spellInfo->Id == 605 ||
                m_spellInfo->Id == 5138 || m_spellInfo->Id == 47857 || m_spellInfo->Id == 47855 || m_spellInfo->Id == 48156 && !m_spellInfo->IsPositive())      // Death Grip  and Curses
                    target->ToTotem()->setDeathState(JUST_DIED);
        ...
        switch (GetSpellInfo()->SpellFamilyName)
        {
            case SPELLFAMILY_GENERIC:
                switch (GetId())
                {
                    case 26013: // Deserter Reduction to 3 minutes
                        SetDuration(180000);
                        break;
                    case 42867:
            ...
            case SPELLFAMILY_WARRIOR:
                ...
                if (GetId() == 47436)
                {
                    int8 ShoutDurationIndex = 2.0f; // 2min
                    
                    if (caster->HasAura(58095)) // Glyph of Battle
                        ShoutDurationIndex += 2.0f;
                        
                    if (caster->GetTypeId() == TYPEID_PLAYER)
                    {
                        if (caster->HasAura(12321))
                            ShoutDurationIndex *= 1.25f; // +25%
                        else if (caster->HasAura(12835))
                            ShoutDurationIndex *= 1.50f; // +50%
                    }
                    if (Aura* aura = caster->GetAura(47436, caster->GetGUID()))
                        aura->SetDuration(1000*60*ShoutDurationIndex);
                    else
                        SetDuration(1000*60*ShoutDurationIndex);
                ...
                if (GetId() == 47440)
                {
                    ...
                    if (caster->HasAura(68164)) // Glyph of Commanding
                        ShoutDurationIndex += 2.0f;
                    ...
                    if (Aura* aura = caster->GetAura(47440, caster->GetGUID()))
                        aura->SetDuration(1000*60*ShoutDurationIndex);
                    else
                        SetDuration(1000*60*ShoutDurationIndex);
    ...
    else
    {
        ...
        if (target && !target->HasAuraType(SPELL_AURA_MOD_CONFUSE))
        {
            target->ClearUnitState(UNIT_STATE_CONFUSED);
            target->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_CONFUSED);
        }
    }
...
void UnitAura::Remove(AuraRemoveMode removeMode)
{
    ...
    if (GetSpellInfo()->Id == 8178) // Grounding totem
        return;
