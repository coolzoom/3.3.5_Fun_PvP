...
bool SpellInfo::IsTrapSpellEffect() const
{
    if (SpellFamilyName == SPELLFAMILY_HUNTER &&
        (SpellFamilyFlags[0] & 0x18 ||     // Freezing and Frost Trap, Freezing Arrow
        Id == 57879 ||                     // Snake Trap - done this way to avoid double proc
        SpellFamilyFlags[2] & 0x00024000)) // Explosive and Immolation Trap
        return true;
...
SpellCastResult SpellInfo::CheckTarget(Unit const* caster, WorldObject const* target, bool implicit) const
{
    ...
    if (!HasAttribute(SPELL_ATTR6_CAN_TARGET_INVISIBLE) && !caster->CanSeeOrDetect(target, implicit) && !IsTrapSpellEffect())
        return SPELL_FAILED_BAD_TARGETS;
    ...
    if ((Effects[0].TargetA.GetTarget() == TARGET_UNIT_CONE_ENEMY_104) && !IsPositive() && !IsPassive())
...
uint32 SpellInfo::CalcCastTime(Spell* spell /*= NULL*/) const
{
    ...
    if (spell && castTime > 0 && spell->GetCaster()->GetTypeId() == TYPEID_PLAYER)
        if (Battleground *bg = spell->GetCaster()->ToPlayer()->GetBattleground())
            if (bg->isArena() && bg->GetStartDelayTime() > 7500)
                return 100;

    if (spell && spell->GetCaster()->GetTypeId() == TYPEID_PLAYER)              
        for (int i = 0; i < MAX_SPELL_EFFECTS; i++)
			switch (this->Effects[i].Effect)
            {
