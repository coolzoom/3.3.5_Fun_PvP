...
void WorldSession::HandleArenaTeamInviteOpcode(WorldPacket& recvData)
{
    ...
    if (arenaTeam->GetMembersSize() >= arenaTeam->GetType() * 2 || arenaTeam->GetType() == ARENA_TEAM_5v5 /*Don't allow invite for 3v3 solo team*/)
    {
...
void WorldSession::HandleArenaTeamLeaveOpcode(WorldPacket& recvData)
{
    ...
    /// Todo: Just tempotary - It makes a unable to leave SOLO team
    /*if (arenaTeam->GetType() == ARENA_TYPE_5v5 || arenaTeam->GetType() == ARENA_TYPE_2v2_SOLO)
    {
        SendArenaTeamCommandResult(ERR_ARENA_TEAM_QUIT_S, "", "", ERR_ARENA_TEAM_INTERNAL);
        _player->GetSession()->SendAreaTriggerMessage("You cannot disband your Solo Queue team");
        return;
    }*/
