...
void WorldSession::HandleBattlemasterJoinOpcode(WorldPacket& recvData)
{
    ...
    if (DisableMgr::IsDisabledFor(DISABLE_TYPE_BATTLEGROUND, bgTypeId_, NULL) && bgTypeId_ != 11)
    {
    ...
    {
        ChatHandler(this).PSendSysMessage("ERROR: You are Desertir.");
        return;    
    }

    if (_player->HasAura(9454))
    {
        ChatHandler(this).PSendSysMessage("ERROR: You are Frozen! Open ticket for request remove aura.");
        return;
	}
    ...
    {
        ChatHandler(this).PSendSysMessage("ERROR: You are in combat.");
        return;    
    }
...
void WorldSession::HandleBattleFieldPortOpcode(WorldPacket &recvData)
{
    ...
    if (type == ARENA_TYPE_5v5 && _player->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_2v2_SOLO))
        type = ARENA_TYPE_2v2_SOLO; // It's hacky but works :)
        
    if (type == ARENA_TYPE_2v2 && _player->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_2v2_SOLO))
        type = ARENA_TYPE_2v2_SOLO; // It's hacky but works :)
...
void WorldSession::HandleBattlemasterJoinArena(WorldPacket& recvData)
{
    ...
    uint8 arenaslot;                                        // 2v2, 3v3 or 5v5 or SoloQue
    ...
    if (_player->getSkirmishStatus(arenatype) != SKIRMISH_PREPEAR) {
        Creature* unit = GetPlayer()->GetMap()->GetCreature(guid);
        if (!unit)
            return;
    ...
    if (bgQueue.IPExistsInQueue(_player, NULL, bracketEntry, arenatype, isRated, false)) 
    {
        //ChatHandler(this).PSendSysMessage("ERROR: You cannot join Arena now...");
        return;
    }
    ...
    if (_player->getSkirmishStatus(arenatype) == SKIRMISH_PREPEAR)
        _player->setSkirmishStatus(arenatype, SKIRMISH_JOINED);
