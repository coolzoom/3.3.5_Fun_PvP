...
void WorldSession::HandleMovementOpcodes(WorldPacket & recvData)
{
    ...
    if (movementInfo.HasMovementFlag(MOVEMENTFLAG_ONTRANSPORT))
    {
        ...
        if (mover->IsSitState() && movementInfo.GetMovementFlags() & (MOVEMENTFLAG_MASK_MOVING | MOVEMENTFLAG_MASK_TURNING) && mover->IsMounted())
       ...
       switch (plrMover->GetMapId())	
        {
            ...
            case 618: underMapValueZ = 26.7f; break; // Ring of Valor
        ...
        if (movementInfo.pos.GetPositionZ() < plrMover->GetMap()->GetMinHeight(movementInfo.pos.GetPositionX(), movementInfo.pos.GetPositionY()))
        {
		    if (underMapValueZ != -500) // For Custom Arena Zone
			{
                ...
                if (plrMover->GetMapId() == 618 && plrMover->GetBattleground()->GetStartTime() > 2 * MINUTE*IN_MILLISECONDS) // Ring of Valor 763.06 -284.15 28
                    plrMover->TeleportTo(618, 763.06f, -284.15f, 30.0f, 1.6f); 
