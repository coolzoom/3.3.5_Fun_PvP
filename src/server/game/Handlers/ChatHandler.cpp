...
void WorldSession::HandleMessagechatOpcode(WorldPacket& recvData)
{
    ...
    {
        QueryResult result5 = LoginDatabase.PQuery("SELECT color FROM account WHERE id = %u", _player->GetSession()->GetAccountId());
        if (result5 && !msg.empty())
        {
            Field *fields = result5->Fetch();
            uint32 color = fields[0].GetUInt32();

            if (color == 1)
                msg = "|cffD00000" + msg + "|r"; // красный
            if (color == 2)
                msg = "|cff3333CC" + msg + "|r"; // синий
            if (color == 3)
                msg = "|cff009900" + msg + "|r"; // зелёный
            if (color == 4)
                msg = "|cff0099FF" + msg + "|r"; // голубой
            if (color == 5)
                msg = "|cffFF99FF" + msg + "|r"; // розовый
            if (color == 7)
                msg = "|cff9192a1" + msg + "|r"; // серый
            if (color == 8)
                msg = "|cffb37700" + msg + "|r"; // коричневый
            if (color == 9)
                msg = "|cffffd9b3" + msg + "|r"; // бежевый
        }
    }
        ...
        if(sWorld->getBoolConfig(CONFIG_WORD_FILTER_ENABLE))
        {
            ...
            if (!badWord.empty())
            {
                ...
                //for log world filter
				//sLog->outDebug(LOG_FILTER_WORD, "[FILTER] Player %s sends: %s",sender->GetName().c_str(), msg.c_str());
				sWorld->SendGMText(LANG_WORD_FILTER_BADCHAT_SENDGM_IN_CHAT, "FILTER",sender->GetName().c_str(), msg.c_str());
                return; 
            }
    ...
    switch (type)
    {
        ...
        case CHAT_MSG_WHISPER:
        {
            ...
            // Remove CrossFrac Realm Mark when whisping (Trying this) 
            /// FIX ME: We need better way, this is temp hackfix
        ...
        case CHAT_MSG_PARTY_LEADER:
        {
            ...
            // if player is in battleground, he cannot say to battleground members by /p - Work in SOLOQUE
            Group* group = GetPlayer()->GetOriginalGroup();
            
            if (Battleground const* bg = sender->GetBattleground())
                if (bg->GetArenaType() == ARENA_TYPE_2v2_SOLO || bg->GetArenaType() == ARENA_TYPE_5v5) 
        ...
        case CHAT_MSG_CHANNEL:
        {
            ...
            if (now - timeLastChannelFlood < 1)
            {
                ChatHandler(sender->GetSession()).PSendSysMessage("AntiFlood: You have sent too many messages in a short time interval.");
                return;
            }
            else
                timeLastChannelFlood = now;
            if (GetPlayer()->SpamCheckForType(type, lang))
                if (!GetPlayer()->DoSpamCheck(msg))
                    return;
