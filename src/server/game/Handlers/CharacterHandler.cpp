...
void WorldSession::HandleCharDeleteOpcode(WorldPacket& recvData)
{
    ...
    if (ArenaTeam* arena = sArenaTeamMgr->GetArenaTeamByCaptain(guid))
    {
        // SOLOQUEUE - Allow to delete character with Solo Queue Team
        if (arena->GetType() < ARENA_TYPE_5v5 || arena->GetType() < ARENA_TYPE_2v2_SOLO)
        {
            sScriptMgr->OnPlayerFailedDelete(guid, initAccountId);
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder* holder)
{
    ...
    uint32 acId = pCurrChar->GetSession()->GetAccountId();
