...
void WorldSession::HandleGroupInviteOpcode(WorldPacket& recvData)
{
    ...
    // Finding player
    /*if (!ObjectAccessor::FindPlayerByName(membername))
        if (Player* pPlayer = GetPlayer()->GetSelectedPlayer())
        {
            if (pPlayer->GetName() == membername || pPlayer->GetRealName() == membername)
                membername = pPlayer->GetName();
        }*/
    ...
    if (player->GetMapId() == 0 && player->GetMapId() == 1 && GetPlayer()->GetTeam() != player->GetTeam())
    {
        SendPartyResult(PARTY_OP_INVITE, membername, ERR_PLAYER_WRONG_FACTION);
        return;
	}
    if (GetPlayer()->GetAreaId() == 616 && player->GetAreaId() == 616)
    {
        SendPartyResult(PARTY_OP_INVITE, membername, ERR_GROUP_FULL);
        return;
    }
    if (GetPlayer()->GetAreaId() == 616 && GetPlayer()->GetTeam() != player->GetTeam())
    {
        Group* plrgroupcheck = GetPlayer()->GetGroup();
		if (plrgroupcheck && plrgroupcheck->GetMembersCount() >= 1)
		{
        	SendPartyResult(PARTY_OP_INVITE, membername, ERR_GROUP_FULL);
        	return;
		}
