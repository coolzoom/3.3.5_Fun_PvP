...
void WorldSession::HandleLogoutRequestOpcode(WorldPacket& /*recvData*/)
{
    ...
    if (ObjectGuid lguid = GetPlayer()->GetLootGUID())
        DoLootRelease(lguid);
    uint32 acId = GetPlayer()->GetSession()->GetAccountId();
    bool instantLogout = (GetPlayer()->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_RESTING) && !GetPlayer()->IsInCombat()) ||
                         GetPlayer()->IsInFlight() || HasPermission(rbac::RBAC_PERM_INSTANT_LOGOUT) || GetPlayer()->GetZoneId() == 394;
    ...
    else if (GetPlayer()->duel || GetPlayer()->HasAura(9454) || GetPlayer()->InArena()) // is dueling or frozen by GM via freeze command and on arenas.
