...
void WorldSession::HandleItemQuerySingleOpcode(WorldPacket& recvData)
{
    ...
    if (GetPlayer() && pProto && pProto->InventoryType != INVTYPE_TABARD && pProto->InventoryType != INVTYPE_AMMO && pProto->InventoryType != INVTYPE_BAG && (pProto->Class == ITEM_CLASS_WEAPON || pProto->Class == ITEM_CLASS_ARMOR))
    {
        //Load custom Ilv. We have custom fce for it, so its best to use it in every case. This is startup load when no cache in wow
        PreparedStatement*  stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_ITEM_INSTANCE_BYOWNER);
        stmt->setUInt64(0, GetPlayer()->GetGUID().GetCounter());
        stmt->setUInt16(1, item);
        ...
        if (guid > 0)
            if (Item* tempitem = GetPlayer()->GetItemByGuid(ObjectGuid(HighGuid::Item, 0, guid)))
                tempIlv = tempitem->GetItemUpdateLevel() > 0 ? tempitem->GetItemUpdateLevel() : 0;

        if (tempIlv > 0)
        {
            if (pProto->ItemLevel > tempIlv)
                diff = pProto->ItemLevel - tempIlv;
            else
                diff = tempIlv - pProto->ItemLevel;
        }
        if (pProto->ItemLevel > tempIlv)
            ost_name << pProto->Name1 << " (-" << diff << ")";
        else
            ost_name << pProto->Name1 << " (+" << diff << ")";
...
void WorldSession::SendListInventory(ObjectGuid vendorGuid, uint32 vendorEntry)
{
    ...
    for (uint8 slot = 0; slot < itemCount; ++slot)
    {
        if (VendorItem const* item = items->GetItem(slot))
        {
            ...
            {
                _player->LoadItemTemplateQuery(0, itemTemplate); // for 0 in future we can implentement vendors with customItemLevel, just retype 0 to GetItemUpgradeLevel etc ..
