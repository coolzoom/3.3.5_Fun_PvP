    ...
    case WARDEN_ACTION_BAN:
        {
            ...
            std::string yel_colour = "F7FE2E";
			std::string plr = _session->GetPlayerName();
			stream << "|CFF" << tag_colour << 
                      "[WARDEN]:|r |cff" << yel_colour << "It was discovered the illegal use of software player [" << plr << "], And blocked."; 
			 sWorld->SendServerMessage(SERVER_MSG_STRING, stream.str().c_str());
            sWorld->BanAccount(BAN_ACCOUNT, accountName, duration.str(), banReason.str(),"Server");
			//sWorld->SendWorldText(SERVER_WARDEN_BAN_ANNOUNCE, accountName.c_str());
            return "Ban";
        }
    default:
        break;
