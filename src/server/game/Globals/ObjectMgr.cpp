...
void ObjectMgr::LoadCreatureTemplates()
{
    ...
    //                                        62           63           64              65            66             67              68                  69
                                             "InhabitType, UsingMmaps, HoverHeight, HealthModifier, ManaModifier, ArmorModifier, DamageModifier, ExperienceModifier, RacialLeader, "
    //                                             70          71                72               73          74
...
void ObjectMgr::LoadCreatureTemplate(Field* fields)
{
    ...
    creatureTemplate.UsingMmaps     = fields[63].GetUInt8();
    ...
    creatureTemplate.ScriptID           = GetScriptId(fields[75].GetCString());
}
...
void ObjectMgr::CheckCreatureTemplate(CreatureTemplate const* cInfo)
{
    ...
    if (cInfo->UsingMmaps <= 0 || cInfo->UsingMmaps > 1)
    {
        //TC_LOG_ERROR("sql.sql", "Creature (Entry: %u) has wrong value (%u) in `UsingMmaps`, creature will ignore mmaps (please fix it).", cInfo->Entry, cInfo->UsingMmaps);
        const_cast<CreatureTemplate*>(cInfo)->UsingMmaps = 0;
    }
