...
void BattlegroundEY::EndBattleground(uint32 winner)
{
    ...
    if (winner == ALLIANCE)
    {
        ...
        std::string msg = "The Battle for Eye of the Storm, has been completed! Alliance win!";
        //sWorld->SendWorldText(LANG_BG_WIN_ANNOUNCE, msg.c_str());
    }
    else if (winner == HORDE)
    {
        RewardHonorToTeam(GetBonusHonorFromKill(1), HORDE);
        std::string msg = "The Battle for Eye of the Storm, has been completed! Horde win!";
        //sWorld->SendWorldText(LANG_BG_WIN_ANNOUNCE, msg.c_str());
    }
    // Complete map reward
    RewardHonorToTeam(GetBonusHonorFromKill(1), ALLIANCE);
