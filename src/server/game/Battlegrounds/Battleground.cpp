...
uint32 buff_updatetimmer = 0;
uint32 VisionTimmer      = 0;
uint8  playerCount       = 0;
...
Battleground::~Battleground()
{
    ...
    // Cleanup temp arena teams for solo 2v2
    if (isArena() && isRated() && GetArenaType() == ARENA_TYPE_2v2_SOLO)
    {
...
void Battleground::Update(uint32 diff)
{
	...
	switch (GetStatus())
	{
        case STATUS_WAIT_JOIN:
            ...
            if (GetArenaType() == ARENA_TYPE_2v2_SOLO || GetArenaType() == ARENA_TYPE_5v5)
            {
                playerCount = 0;
                ...
                if (playerCount == 6)
                {
                    ...
                    if (VisionTimmer >= 8000)
                        for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
                            if (Player* player = ObjectAccessor::FindPlayer(itr->first))
                            {
                                player->RemoveAurasDueToSpell(SPELL_ARENA_PREPARATION);
                                player->CastSpell(player, SPELL_ARENA_PREPARATION_SOLO, true);
                                Preparation = false;
                            }
                }
            }
        ...
        case STATUS_IN_PROGRESS:
            ...
            if (isArena())
            {
                if (buff_updatetimmer > 1000)
                {
                    if (GetStartTime() >= ((5+BuffStacks) * MINUTE*IN_MILLISECONDS))
                    {
                        for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
                            if (Player* player = ObjectAccessor::FindPlayer(itr->first))
                                if (player && player->IsAlive() && !player->IsSpectator())
                                    player->AddAura(29659, player);
                        BuffStacks++;
                    }
                    buff_updatetimmer = 0;
                }
                if (GetStartTime() >= 30 * MINUTE*IN_MILLISECONDS)
                {
                    BuffStacks = 0;
                    EndBattleground(0);
                    return;
...
void Battleground::CheckStartSolo2v2Arena()
{
    if (GetArenaType() != ARENA_TYPE_2v2_SOLO)
        return;
    ...
    for (int i = 0; i < 2; i++)
    {
        for (ArenaTeam::MemberList::iterator itr = team[i]->m_membersBegin(); itr != team[i]->m_membersEnd(); itr++)
        {
            ...
            {
                //if (sWorld->getBoolConfig(CONFIG_SOLO_2V2_CAST_DESERTER_ON_AFK))
    ...
    if (someoneNotInArena/* && sWorld->getBoolConfig(CONFIG_SOLO_2V2_STOP_GAME_INCOMPLETE)*/ || plrinarena < 6)
    {
        SetRated(false);
        EndBattleground(0);
    }        
...
void Battleground::EndBattleground(uint32 winner)
{
	...
	for (BattlegroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
	{
        ...
        if (sWorld->getBoolConfig(CONFIG_SOLO_3V3_REWARD))
        {
            if (team == winner)
            {
                if (isArena() && isRated() && GetArenaType() == ARENA_TYPE_5v5)
                    player->AddItem(sWorld->getIntConfig(CONFIG_SOLO_3V3_REWARD_1V1), (sWorld->getIntConfig(CONFIG_SOLO_3V3_REWARD_1V1_COUNT)));
                else if (isArena() && isRated() && GetArenaType() == ARENA_TYPE_2v2_SOLO)
                    player->AddItem(sWorld->getIntConfig(CONFIG_SOLO_3V3_REWARD_3V3), (sWorld->getIntConfig(CONFIG_SOLO_3V3_REWARD_3V3_COUNT)));
                if (Arena1v1CheckTalents(player) == false) 
                    player->AddItem(sWorld->getIntConfig(CONFIG_SOLO_3V3_REWARD_HEALER), (sWorld->getIntConfig(CONFIG_SOLO_3V3_REWARD_HEALER_COUNT)));
                else if (isArena() && isRated() && GetArenaType() == ARENA_TYPE_2v2)
                    player->AddItem(sWorld->getIntConfig(CONFIG_SOLO_3V3_REWARD_2V2), (sWorld->getIntConfig(CONFIG_SOLO_3V3_REWARD_2V2_COUNT)));
                else if (!isArena())
                    player->AddItem(sWorld->getIntConfig(CONFIG_SOLO_3V3_REWARD_2V2_LOS), (sWorld->getIntConfig(CONFIG_SOLO_3V3_REWARD_2V2_LOS_COUNT)));
            }
            else if (!isArena())
                player->AddItem(sWorld->getIntConfig(CONFIG_SOLO_3V3_REWARD_LOS), (sWorld->getIntConfig(CONFIG_SOLO_3V3_REWARD_LOS_COUNT)));
        }
		...
		if (!spectPacketSent && isArena())
		{
			SpectatorList::const_iterator citer = m_Spectators.begin();
			for (; citer != m_Spectators.end(); ++citer)
			{
				if (Player* tmpPlr = ObjectAccessor::FindPlayer(ObjectGuid(HighGuid::Player, *citer)))
				{
					for (BattlegroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
					{
					...
					sBattlegroundMgr->BuildBattlegroundStatusPacket(&tableData, this, player->GetBattlegroundQueueIndex(bgQueueTypeId), STATUS_IN_PROGRESS, TIME_TO_AUTOREMOVE, GetStartTime(), GetArenaType(), player->GetTeam());
...
void Battleground::RemovePlayerAtLeave(ObjectGuid guid, bool Transport, bool SendPacket)
{
	...
	if (player)
	{
		if (player->getSkirmishStatus(m_ArenaType) == SKIRMISH_JOINED)
			player->setSkirmishStatus(m_ArenaType, SKIRMISH_NONE);
        ...
        player->RemoveAurasDueToSpell(29659); // +Damage in Arena
	...
	if (isArena())
	{
		if (GetStatus() != STATUS_IN_PROGRESS)
		{
			if (player->getClass() == CLASS_MAGE)
				player->CastSpell(player, 58661, true); // Mage's eat
			else if (player->getClass() == CLASS_WARLOCK)
				player->CastSpell(player, 58889, true); // Soulwell
		}
...
uint8 Battleground::ClickFastStart(Player *player, GameObject *go)
{
	...
	std::set<ObjectGuid>::iterator pIt = m_playersWantsFastStart.find(player->GetGUID());
	...
	case ARENA_TYPE_3v3:
		playersNeeded = 6;
		break;
	/*case ARENA_TYPE_2v2_SOLO: // 3v3 Solo System
		playersNeeded = 6;
		break;*/
	case ARENA_TYPE_5v5: // 1v1 Solo System
		playersNeeded = 2;
        if (player->getSkirmishStatus(m_ArenaType) == SKIRMISH_JOINED)
            playersNeeded = 2;
		break;
