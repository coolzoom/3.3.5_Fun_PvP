...
bool ArenaTeam::Create(ObjectGuid captainGuid, uint8 type, std::string const& teamName, uint32 backgroundColor, uint8 emblemStyle, uint32 emblemColor, uint8 borderStyle, uint32 borderColor)
{
    ...
    if (type != ARENA_TYPE_5v5 && type != ARENA_TYPE_2v2_SOLO)
    ...
    Type = type;
    TeamName = (type == ARENA_TEAM_5v5) ?"[SoloQ 3v3]: " + teamName : teamName;
    BackgroundColor = backgroundColor;
    EmblemStyle = emblemStyle;
    EmblemColor = emblemColor;
    BorderStyle = borderStyle;
    BorderColor = borderColor;
    Stats.Rating = (type == ARENA_TEAM_5v5) ? 1000 : sWorld->getIntConfig(CONFIG_ARENA_START_RATING);
...
void ArenaTeam::CreateTempForSolo2v2(Player* plr[], uint8 team)
{
    // Generate new arena team id
	...
	std::string teamType = "TEAM: ";
		
    ssTeamName << "Team " << (team + 1);
    ...
    for (int i = 0; i < PLAYERS_COUNT_PER_TEAM; i++)
    {
...
bool ArenaTeam::AddMember(ObjectGuid playerGuid)
{
    ...
    if (GetType() == ARENA_TYPE_5v5 && GetMembersSize() == 1)
        return false;
...
{
    if (int32(PersonalRating) + mod < 500)
        PersonalRating = (type == ARENA_TEAM_5v5) ? 1500 : 1000;
    else
        PersonalRating += mod;
    ...
    if (player)
    {
        ...
        player->GetHighestArenaRating(type, true);
    }
}
...
uint32 ArenaTeam::GetPoints(uint32 memberRating)
{
    ...
    */
    if (rating <= 1500)
         points = irand(200,300);
    else
        points = irand(200,300) + 0.28 * (float)rating;

    if (Type == ARENA_TEAM_5v5 || Type == ARENA_TEAM_SOLO_2v2)
        points *= 0.45f;
...
int32 ArenaTeam::GetMatchmakerRatingMod(uint32 ownRating, uint32 opponentRating, bool won /*, float& confidence_factor*/)
{
    ...
    */
    ...
    // Real mmr modification
    /*if (won)
    {
		if (opponentRating > ownRating)
            mod = 16.0f * (won_mod - chance) + (0.025f * (opponentRating - ownRating));
		else
        {
            mod = 24.0f * (won_mod - chance) - (0.008f * (ownRating - opponentRating));
            if (mod < 0)
                mod = 1.0f;
        }
    }
    ...
    {
        if (ownRating > opponentRating)
            mod = (24.0f * (won_mod - chance)) - (0.013f * (ownRating - opponentRating));
        else
...
int32 ArenaTeam::GetRatingMod(uint32 ownRating, uint32 opponentRating, bool won /*, float confidence_factor*/)
{
    ...
    if (won)
    {
		...
		else if (opponentRating > ownRating)
		{
            if (ownRating < 1500)
                mod = 26.0f * (won_mod - chance) + (0.018f * (opponentRating - ownRating));
            else
                mod = 16.0f * (won_mod - chance) + (0.025f * (opponentRating - ownRating));
		}
		else
        {
            mod = 24.0f * (won_mod - chance) - (0.008f * (ownRating - opponentRating));
            if (mod < 0)
                mod = 1.0f; // +1 if very big difference between ratings
        }
    }
    else
    {
    ...
    else
    {
        ...
        for (MemberList::iterator itr = Members.begin(); itr != Members.end(); ++itr)
            if (Player* member = ObjectAccessor::FindConnectedPlayer(itr->Guid))
            {
                ...
                member->GetHighestArenaRating(Type, true); // Update fce
            }
    }
...
void ArenaTeam::SaveToDB()
{
    ...
    for (MemberList::const_iterator itr = Members.begin(); itr != Members.end(); ++itr)
    {
        ...
        for (ArenaTeamMgr::ArenaTeamContainer::const_iterator itrMgr = sArenaTeamMgr->GetArenaTeamMapBegin(); itrMgr != sArenaTeamMgr->GetArenaTeamMapEnd(); itrMgr++)
        {
    ...
    for (MemberList::const_iterator itr = Members.begin(); itr != Members.end(); ++itr)
    {
        ...
        if (!plrArenaTeam)
         ...
         if (ratingModifier > 60 || ratingModifier < -60)
        {
            // This should never happen. Maybe output an error message?
            TC_LOG_ERROR("bg.arena", "Solo 3v3: Error while saving new rating. ratingModifier: %i, ArenaTeam: %s", ratingModifier, plrArenaTeam->GetName().c_str());
            continue;
        }
        ...
        for (MemberList::iterator realMemberItr = plrArenaTeam->Members.begin(); realMemberItr != plrArenaTeam->Members.end(); ++realMemberItr)
        {
            ...
            {
                realMemberItr->PersonalRating = plrArenaTeam->Stats.Rating;
...
{
    // Moved from SaveToDB() to SaveToDBHelper() for solo 2v2
