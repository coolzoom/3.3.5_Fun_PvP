...
void BattlegroundMgr::Update(uint32 diff)
{
    ...
    if (sWorld->getBoolConfig(CONFIG_ARENA_AUTO_DISTRIBUTE_POINTS))
    {
        if (m_AutoDistributionTimeChecker < diff)
        {
            ...
            if (time(NULL) > m_NextAutoDistributionTime && timeinfo->tm_hour >= 20)
            {   
...
void BattlegroundMgr::BuildBattlegroundStatusPacket(WorldPacket* data, Battleground* bg, uint8 QueueSlot, uint8 StatusID, uint32 Time1, uint32 Time2, uint8 arenatype, uint32 arenaFaction)
{
    ...
    *data << uint8(arenatype == 4 ? 3 : arenatype);
    ...
    switch (bgTypeId)
    {
        ...
        case BATTLEGROUND_RV:
            switch (arenaType)
            {
                ...
                case ARENA_TYPE_2v2_SOLO:
                    return BATTLEGROUND_QUEUE_2v2_SOLO;
                default:
...
void BattlegroundMgr::ScheduleQueueUpdate(uint32 arenaMatchmakerRating, uint8 arenaType, BattlegroundQueueTypeId bgQueueTypeId, BattlegroundTypeId bgTypeId, BattlegroundBracketId bracket_id, uint8 dynamicMMRindex)
{
            ...
            for (int bgTypeId = 0; bgTypeId < MAX_BATTLEGROUND_BRACKETS; ++bgTypeId)
            {
                for (int groupType = BG_QUEUE_PREMADE_ALLIANCE; groupType < BG_QUEUE_NORMAL_ALLIANCE; ++groupType)
                {
                        ...
                        for (; itr != m_BattlegroundQueues[i].m_QueuedGroups[bgTypeId][groupType].end(); itr++)
                        {
                            ...
                            {
                                if (getMSTime() - (*itr)->JoinTime > 600000)
                                {
                                    if ((*itr)->DynamicMMRindex < 6)
                                    {
                                        ...
                                        if (ArenaTeam* team = sArenaTeamMgr->GetArenaTeamById((*itr)->ArenaTeamId))
                                        {
                                            ...
                                            {
                                                //if (Player* plr = ObjectAccessor::FindPlayer(iter->Guid))
                                                    //ChatHandler(plr->GetSession()).PSendSysMessage("QUEUE: finding mmr is between %u MMR - %u MMR", (*itr)->ArenaMatchmakerRating - (600 + GetMaxRatingDifference()), (*itr)->ArenaMatchmakerRating + (600 + GetMaxRatingDifference()));
                                            }
                                        }
                                    }
                                    else
                                ...
                                else if (getMSTime() - (*itr)->JoinTime > 550000)
                                {
                                    if ((*itr)->DynamicMMRindex < 5)
                                    {
                                        ...
                                        if (ArenaTeam* team = sArenaTeamMgr->GetArenaTeamById((*itr)->ArenaTeamId))
                                        {
                                            ...
                                            {
                                                //if (Player* plr = ObjectAccessor::FindPlayer(iter->Guid))
                                                        //ChatHandler(plr->GetSession()).PSendSysMessage("QUEUE: finding mmr is between %u MMR - %u MMR", (*itr)->ArenaMatchmakerRating - (500 + GetMaxRatingDifference()), (*itr)->ArenaMatchmakerRating + (500 + GetMaxRatingDifference()));
                                            }
                                        }
                                    }
                                    else
                                ...
                                else if (getMSTime() - (*itr)->JoinTime > 400000)
                                {
                                    if ((*itr)->DynamicMMRindex < 4)
                                    {
                                        ...
                                        if (ArenaTeam* team = sArenaTeamMgr->GetArenaTeamById((*itr)->ArenaTeamId))
                                        {
                                            ...
                                            {
                                                //if (Player* plr = ObjectAccessor::FindPlayer(iter->Guid))
                                                    //ChatHandler(plr->GetSession()).PSendSysMessage("QUEUE: finding mmr is between %u MMR - %u MMR", (*itr)->ArenaMatchmakerRating - (400 + GetMaxRatingDifference()), (*itr)->ArenaMatchmakerRating + (400 + GetMaxRatingDifference()));
                                            }
                                        }
                                    }
                                    else
                                ...
                                else if (getMSTime() - (*itr)->JoinTime > 250000)
                                {
                                    if ((*itr)->DynamicMMRindex < 3)
                                    {
                                        ...
                                        if (ArenaTeam* team = sArenaTeamMgr->GetArenaTeamById((*itr)->ArenaTeamId))
                                        {
                                            ...
                                            {
                                               // if (Player* plr = ObjectAccessor::FindPlayer(iter->Guid))
                                                    //ChatHandler(plr->GetSession()).PSendSysMessage("QUEUE: finding mmr is between %u MMR - %u MMR", (*itr)->ArenaMatchmakerRating - (300 + GetMaxRatingDifference()), (*itr)->ArenaMatchmakerRating + (300 + GetMaxRatingDifference()));
                                            }
                                        }
                                    }
                                    else
                                ...
                                else if (getMSTime() - (*itr)->JoinTime > 100000)
                                {
                                    if ((*itr)->DynamicMMRindex < 2)
                                    {
                                        ...
                                        if (ArenaTeam* team = sArenaTeamMgr->GetArenaTeamById((*itr)->ArenaTeamId))
                                        {
                                            ...
                                            {
                                                //if (Player* plr = ObjectAccessor::FindPlayer(iter->Guid))
                                                    //ChatHandler(plr->GetSession()).PSendSysMessage("QUEUE: finding mmr is between %u MMR - %u MMR", (*itr)->ArenaMatchmakerRating - (200 + GetMaxRatingDifference()), (*itr)->ArenaMatchmakerRating + (200 + GetMaxRatingDifference()));
                                            }
                                        }
                                    }
                                    else
                                ...
                                else if (getMSTime() - (*itr)->JoinTime > 50000)
                                {
                                    if ((*itr)->DynamicMMRindex < 1)
                                    {
                                        ...
                                        if (ArenaTeam* team = sArenaTeamMgr->GetArenaTeamById((*itr)->ArenaTeamId))
                                        {
                                            ...
                                            {
                                                //if (Player* plr = ObjectAccessor::FindPlayer(iter->Guid))
                                                    //ChatHandler(plr->GetSession()).PSendSysMessnding mmr is between %u MMR - %u MMR", (*itr)->ArenaMatchmakerRating - (100 + GetMaxRatingDifference()), (*itr)->ArenaMatchmakerRating + (100 + GetMaxRatingDifference()));
                                            }
                                        }
                                    }
                                    else
