...
bool BattlegroundQueue::CheckIPExistsInQueue(std::string const& remote_addr, PvPDifficultyEntry const* bracketEntry, uint8 arenaType, bool isRated, bool isPremade)
{
    ...
    for (GroupsQueueType::iterator queue_itr = m_QueuedGroups[bracketId][index].begin(); queue_itr != m_QueuedGroups[bracketId][index].end(); ++queue_itr)
        for (std::map<ObjectGuid, PlayerQueueInfo*>::iterator group_itr = (*queue_itr)->Players.begin(); group_itr != (*queue_itr)->Players.end(); ++group_itr)
...
GroupQueueInfo* BattlegroundQueue::AddGroup(Player* leader, Group* grp, BattlegroundTypeId BgTypeId, PvPDifficultyEntry const*  bracketEntry, uint8 ArenaType, bool isRated, bool isPremade, uint32 ArenaRating, uint32 MatchmakerRating, uint32 arenateamid)
{
    ...
    ginfo->IsCustomSkirmish          = (leader->getSkirmishStatus(ArenaType) == SKIRMISH_JOINED);
    ...
    if (isRated/* && sWorld->getBoolConfig(CONFIG_ARENA_QUEUE_ANNOUNCER_ENABLE)*/)
    {
        ArenaTeam* Team = sArenaTeamMgr->GetArenaTeamById(arenateamid);
            int8 type = ginfo->ArenaType;
        if (Team && ginfo->ArenaType != ARENA_TYPE_2v2_SOLO && ginfo->ArenaType != ARENA_TYPE_5v5)
            sWorld->SendWorldText(LANG_ARENA_QUEUE_ANNOUNCE_WORLD_JOIN, Team->GetName().c_str(), type, type, ginfo->ArenaTeamRating, ginfo->ArenaMatchmakerRating);
    }
    ...
    //add GroupInfo to m_QueuedGroups
    {
        ...
        if (!isRated && !isPremade && !ArenaType && sWorld->getBoolConfig(CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_ENABLE))
        {
            if (Battleground* bg = sBattlegroundMgr->GetBattlegroundTemplate(ginfo->BgTypeId))
            {
                if (sWorld->getBoolConfig(BATTLEGROUND_CROSSFACTION_ENABLED))
                {
                    const std::string bgName = bg->GetName();
...
void BattlegroundQueue::RemovePlayer(ObjectGuid guid, bool decreaseInvitedCount)
{
    ...
    /*if (group->ArenaType && group->IsRated && group->Players.empty())
        ...
        {
			int8 type = group->ArenaType;
			if (group->ArenaType == ARENA_TYPE_2v2_SOLO)
                type = 2;
                
			if (group->ArenaType == ARENA_TYPE_5v5)
                type = 1;
                
            if (group->ArenaType == ARENA_TYPE_2v2_SOLO)
               sWorld->SendWorldText(LANG_ARENA_QUEUE_ANNOUNCE_WORLD_EXIT_SOLO, Team->GetName().c_str(), group->ArenaTeamRating);
            else
               sWorld->SendWorldText(LANG_ARENA_QUEUE_ANNOUNCE_WORLD_EXIT, Team->GetName().c_str(), type, type, group->ArenaTeamRating);
        }*/
...
bool BattlegroundQueue::CheckNormalMatch(Battleground* bg_template, BattlegroundBracketId bracket_id, uint32 minPlayers, uint32 maxPlayers)
{
    ...
    if (bg_template->GetArenaType() == ARENA_TYPE_5v5)
        minPlayers = 2;
...
bool BattlegroundQueue::CheckSolo2v2Arena(BattlegroundBracketId bracket_id)
{
    ...
    bool filterTalents = sWorld->getBoolConfig(CONFIG_SOLO_2V2_FILTER_TALENTS);
    ...
    for (int teamId = 0; teamId < 2; teamId++) // BG_QUEUE_PREMADE_ALLIANCE and BG_QUEUE_PREMADE_HORDE
    {
        for (GroupsQueueType::iterator itr = m_QueuedGroups[bracket_id][teamId].begin(); itr != m_QueuedGroups[bracket_id][teamId].end(); ++itr)
        {
            ...
            for (std::map<ObjectGuid, PlayerQueueInfo*>::iterator grpPlrItr = grpPlr->begin(); grpPlrItr != grpPlr->end(); grpPlrItr++)
            {
                ...
                if (!filterTalents && m_SelectionPools[TEAM_ALLIANCE].GetPlayerCount() + m_SelectionPools[TEAM_HORDE].GetPlayerCount() == REQUIRED_TOTAL_PLAYERS)
                    return true;
                ...
                if (filterTalents && soloTeam[TEAM_ALLIANCE][plrCat] == false // is slot free in alliance team?
                    || (!filterTalents && m_SelectionPools[TEAM_ALLIANCE].GetPlayerCount() != PLAYERS_COUNT_PER_TEAM)) 
                {
                    if (m_SelectionPools[TEAM_ALLIANCE].AddGroup((*itr), PLAYERS_COUNT_PER_TEAM)) // added successfully?
                    {
                        ...
                        {
                            (*itr)->Team = ALLIANCE;
                            m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE].push_back((*itr));
                            itr = m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_HORDE].erase(itr);
                            return CheckSolo2v2Arena(bracket_id);
                        }
                    }
                }
                else if (filterTalents && soloTeam[TEAM_HORDE][plrCat] == false || !filterTalents) // nope? and in horde team?
                {
                    if (m_SelectionPools[TEAM_HORDE].AddGroup((*itr), PLAYERS_COUNT_PER_TEAM))
                    {
                        ...
                        {
                            (*itr)->Team = HORDE;
                            m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_HORDE].push_back((*itr));
    ...
    return countAll == REQUIRED_TOTAL_PLAYERS || 
        (!filterTalents && m_SelectionPools[TEAM_ALLIANCE].GetPlayerCount() + m_SelectionPools[TEAM_HORDE].GetPlayerCount() == REQUIRED_TOTAL_PLAYERS);
}
...
void BattlegroundQueue::CreateTempArenaTeamForQueue(ArenaTeam *arenaTeams[])
{
    ...
    for (uint32 i = 0; i < BG_TEAMS_COUNT; i++)
    {
		...
		Player* atPlr[PLAYERS_COUNT_PER_TEAM];
        uint32 atPlrItr = 0;
        ...
        {
			if (atPlrItr >= PLAYERS_COUNT_PER_TEAM)
        ...
        tempArenaTeam->CreateTempForSolo2v2(atPlr, i);
...
void BattlegroundQueue::BattlegroundQueueUpdate(uint32 /*diff*/, BattlegroundTypeId bgTypeId, BattlegroundBracketId bracket_id, uint8 arenaType, bool isRated, uint32 arenaRating, uint8 dynamicMMRindex)
{
    ...
    else if (arenaType == ARENA_TYPE_2v2_SOLO)
    {
        // Solo 2v2
        if (CheckSolo2v2Arena(bracket_id))
        {
            ...
            //set rating range
			uint32 arenaMinRating = (arenaTeams[TEAM_HORDE]->GetAverageMMR() <= sBattlegroundMgr->GetMaxRatingDifference()) ? 0 : arenaTeams[TEAM_HORDE]->GetAverageMMR() - sBattlegroundMgr->GetMaxRatingDifference();
            uint32 arenaMaxRating = arenaTeams[TEAM_HORDE]->GetAverageMMR() + sBattlegroundMgr->GetMaxRatingDifference();
			
            // Dynamic MMR author : Tyrkysovej
            switch (dynamicMMRindex)
            {
                case 1:
                {
                    arenaMinRating -= 100;
            ...
            if (arenaTeams[TEAM_ALLIANCE]->GetAverageMMR() <  arenaMinRating || arenaTeams[TEAM_ALLIANCE]->GetAverageMMR() > arenaMaxRating)
				return;
			...
			//sWorld->SendWorldText(LANG_ANNOUNCE_COLOR, "MMR Checked and work, creating BG start BG");
    ...
    else if (bg_template->isArena())
    {
        ...
        //p.s. here maybe? we need to find 2 teams which will play next game
        GroupsQueueType::iterator itr_teams[BG_TEAMS_COUNT];
        uint8 found = 0;
        uint8 team = 0;
        uint8 soloTeam = 0;
        //if (arenaType != ARENA_TYPE_SOLO)
        //{
...
bool BGQueueRemoveEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/)
{
    ...
    if (queueSlot < PLAYER_MAX_BATTLEGROUND_QUEUES)         // player is in queue, or in Battleground
    {
        ...
        if (bgQueue.IsPlayerInvited(m_PlayerGuid, m_BgInstanceGUID, m_RemoveTime))
        {
            ...
            if (bg && sBattlegroundMgr->IsArenaType(m_BgTypeId))
            {
                if (bg->GetStatus() != STATUS_WAIT_LEAVE)
                {
                        player->CastSpell(player, 26013, true); // deserter
                    // Notify others about player leaving and skip rating
                    {
                        bg->SendWarningToAll(3, "The match will be ended and will not be rated. A player did not accept the invite");
                        bg->SetRated(false);
