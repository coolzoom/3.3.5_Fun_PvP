...
void SpectatorAddonMsg::RemoveAura(ObjectGuid _caster, uint32 _spellId, bool _isDebuff, uint8 _type, int32 _duration, int32 _expire, uint16 _stack, bool _isRemove)
{
    if (!CanSandAura(_spellId))
        return;
    ...
    aStack      = _stack;
    aRemove     = _isRemove;
    EnableFlag(SPECTATOR_PREFIX_AURA_REMOVE);
}
    ...
    for (uint8 i = 0; i < SPECTATOR_PREFIX_COUNT; ++i)
        if (isFilledIn(i))
        {
            switch (i)
            {
                ...
                {
                    char buffer[300];
                    sprintf(buffer, "ARE=%i,%i,%i,%i,%i,%i,%i,0x%X;", aRemove,   aStack,
                                                                    aExpire,   aDuration,
                                                                    aSpellId,  aType,
                                                                    aIsDebuff, aCaster);
                    msg += buffer;
