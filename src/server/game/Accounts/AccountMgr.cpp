...
/*void AccountMgr::DeleteTrialAccounts()
{

    QueryResult result = LoginDatabase.PQuery("SELECT id FROM trial_accounts");
    uint32 deleted = 0;
    if (result)
    {
        do
        {
			...
			if (GetPassedDaysForTrial(id) >= DAYS_FOR_TRIAL)
                if (DeleteAccount(id) == AOR_OK)
                {
                    LoginDatabase.PQuery("DELETE FROM trial_accounts WHERE id = %u", id);
                    deleted++;
                }

        } while (result->NextRow());
    }
    //Log->outInfo("Deleted trial accounts: %u.", deleted);
}

int32 AccountMgr::GetPassedDaysForTrial(uint32 accountId)
{
    int32 daysLeft = 0;
    QueryResult result = LoginDatabase.PQuery("SELECT created FROM trial_accounts WHERE id = '%u'", accountId);
    if (result)
    {
        std::string created = (*result)[0].GetString();
        uint32 year, month, day;
        sscanf(created.c_str(), "%4u%2u%2u", &year, &month, &day);
        ...
        double diff = difftime(time(NULL), rawtime);

        daysLeft = int(int(diff / 60) / 60) / 24;
    }
