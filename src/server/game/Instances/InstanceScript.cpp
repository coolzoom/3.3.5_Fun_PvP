...
void InstanceScript::DoCompleteAchievement(uint32 achievement)
{
    ...
    if (!pAE)
    {
        TC_LOG_ERROR("misc", "TSCR: DoCompleteAchievement called for not existing achievement %u", achievement);
        return;
    }
...
uint32 InstanceScript::GetCompletedEncountersReadable() const
{
    uint32 completedEncountersDecimal = 0;
    uint8 num = GetCompletedEncounterMask();
    uint8 num_aux = GetCompletedEncounterMask();
    uint8 i;
    uint8 j;
    uint8 res;
    for(i = 1 ; i <= num ; i++) // Sets number of divisions
    {
        num_aux = num_aux / 2;
        if(num_aux == 1)
            break;
    }
    for(j = 1 ; j <= i; j++) // Conversion
    {
        res = uint8(num % 2);
        if(res == 1)
            completedEncountersDecimal++;
        num = num / 2;
    }
    if (num > 0)
        completedEncountersDecimal++;

    return completedEncountersDecimal;
}

uint32 InstanceScript::GetTotalEncountersReadable() const
{
    // forced due to heroic raids does not save data and calling it leads to crash inmediately
    Difficulty actualDifficulty = instance->GetDifficulty();

    if (actualDifficulty == RAID_DIFFICULTY_10MAN_HEROIC)
        actualDifficulty = RAID_DIFFICULTY_10MAN_NORMAL;
    else if (actualDifficulty == RAID_DIFFICULTY_25MAN_HEROIC)
        actualDifficulty = RAID_DIFFICULTY_25MAN_NORMAL;

    return uint32(sObjectMgr->GetDungeonEncounterList(instance->GetId(), actualDifficulty)->size());
}
