...
enum Spells
{
    SPELL_DRAIN_LIFE            = 20743, // DrainLife
...
enum Events
{
    ...
    EVENT_ASPECT_OF_JEKLIK      = 7,
    EVENT_ASPECT_OF_VENOXIS     = 8,
    EVENT_ASPECT_OF_MARLI       = 9,
    EVENT_ASPECT_OF_THEKAL      = 10,
    EVENT_ASPECT_OF_ARLOKK      = 11
};
...
class boss_hakkar : public CreatureScript
{
    public:
        ...
        struct boss_hakkarAI : public BossAI
        {
            ...
            {
                _EnterCombat();
                events.ScheduleEvent(EVENT_BLOOD_SIPHON, 60000);
                events.ScheduleEvent(EVENT_CORRUPTED_BLOOD, 25000);
                events.ScheduleEvent(EVENT_CAUSE_INSANITY, 30000);
                events.ScheduleEvent(EVENT_WILL_OF_HAKKAR, 17000);
                events.ScheduleEvent(EVENT_ENRAGE, 600000);
                events.ScheduleEvent(EVENT_DRAIN_LIFE, 30000);
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_DRAIN_LIFE:
                            DoCastVictim(SPELL_DRAIN_LIFE, true);
                            events.ScheduleEvent(EVENT_DRAIN_LIFE, 30000);
                            break;
                        ...
                        case EVENT_CORRUPTED_BLOOD:
                            ...
                            events.ScheduleEvent(EVENT_CORRUPTED_BLOOD, urand(20000, 45000));
                            break;
                        case EVENT_CAUSE_INSANITY:
                             DoCast(SelectTarget(SELECT_TARGET_RANDOM, 0, 100, true), SPELL_CAUSE_INSANITY);
                             events.ScheduleEvent(EVENT_CAUSE_INSANITY, urand(75000, 75000));
                            break;
                        case EVENT_WILL_OF_HAKKAR:
                            DoCast(SelectTarget(SELECT_TARGET_RANDOM, 0, 100, true), SPELL_WILL_OF_HAKKAR);
                            events.ScheduleEvent(EVENT_WILL_OF_HAKKAR, urand(25000, 45000));
                            break;
                        case EVENT_ENRAGE:
