                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_POISON_VOLLEY:
                            ...
                            if (me->HasAura(SPELL_SPIDER_FORM))
                                events.ScheduleEvent(EVENT_POISON_VOLLEY, urand(4000, 8000));
                            else
                                events.ScheduleEvent(EVENT_POISON_VOLLEY, urand(8000, 16000));
                            break;
                        ...
                        case EVENT_SPAWN_SPIDER:
                            if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                                if (Creature* spider = me->SummonCreature(NPC_SPIDER, target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 15000))
                                    spider->AI()->AttackStart(target);
                            events.ScheduleEvent(EVENT_SPAWN_SPIDER, urand(40000, 50000));
                            break;
                        case EVENT_TRANSFORM:
                        {
                            ...
                            */
                            me->HandleStatModifier(UNIT_MOD_DAMAGE_MAINHAND, TOTAL_PCT, 6.0f, true); // hack
                            DoCast(SelectTarget(SELECT_TARGET_RANDOM, 1, 100, true), SPELL_ENVOLWINGWEB);
                        ...
                        case EVENT_TRANSFORM_BACK:
                        {
                            ...
                            me->UpdateDamagePhysical(BASE_ATTACK);
                            */
                            me->HandleStatModifier(UNIT_MOD_DAMAGE_MAINHAND, TOTAL_PCT, 6.0f, false); // hack
                            events.ScheduleEvent(EVENT_ASPECT_OF_MARLI, 12000, 0, PHASE_TWO);
                            events.ScheduleEvent(EVENT_TRANSFORM, 45000, 0, PHASE_TWO);
                            events.ScheduleEvent(EVENT_POISON_VOLLEY, 15000);
                            events.ScheduleEvent(EVENT_SPAWN_SPIDER, 75000);
