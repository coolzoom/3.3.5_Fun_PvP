...
class boss_hazzarah : public CreatureScript
{
    public:
        ...
        struct boss_hazzarahAI : public BossAI
        {
            ...
            {
                _EnterCombat();
                events.ScheduleEvent(EVENT_MANABURN, urand(4000, 8000));
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_MANABURN:
                            ...
                            events.ScheduleEvent(EVENT_MANABURN, urand(12000, 16000));
                            break;
                        case EVENT_SLEEP:
                            DoCast(SelectTarget(SELECT_TARGET_RANDOM, 0, 40, true), SPELL_SLEEP, true);
