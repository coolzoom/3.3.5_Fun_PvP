                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_HOLY_WRATH:
                            ...
                            events.ScheduleEvent(EVENT_HOLY_WRATH, urand(15000, 20000), 0, PHASE_ONE);
                            break;
                        ...
                        case EVENT_VENOM_SPIT:
                            ...
                            events.ScheduleEvent(EVENT_VENOM_SPIT, urand(5000, 10000), 0, PHASE_TWO);
                            break;
                        case EVENT_POISON_CLOUD:
                            //if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM))
                                DoCastVictim(SPELL_POISON_CLOUD);
                            events.ScheduleEvent(EVENT_POISON_CLOUD, urand(4000, 6000), 0, PHASE_TWO);
                            break;
                        case EVENT_PARASITIC_SERPENT:
                            if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM))
                                DoCast(target, SPELL_SUMMON_PARASITIC_SERPENT);
                            events.ScheduleEvent(EVENT_PARASITIC_SERPENT, 5000, 0, PHASE_TWO);
                            break;
                        case EVENT_FRENZY:
                            // frenzy at 20% health
