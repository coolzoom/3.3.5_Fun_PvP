    ...
    public:
        ...
        struct boss_azuregosAI : public WorldBossAI
        {
            ...
            void EnterCombat(Unit* /*who*/) override
            {
				...
				sWorld->SendWorldText(LANG_ANNOUNCE_COLOR, "Azuregos", "I'm under attack!");
                events.ScheduleEvent(EVENT_MARK_OF_FROST, 35000);
                events.ScheduleEvent(EVENT_MANA_STORM, urand(5000, 17000));
                events.ScheduleEvent(EVENT_CHILL, urand(8000, 30000));
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_MANA_STORM:
                            ...
                            if (_enraged)
                                events.ScheduleEvent(EVENT_MANA_STORM, urand(4500, 8000));
                            else
                                events.ScheduleEvent(EVENT_MANA_STORM, urand(7500, 12500));
                            break;
                        case EVENT_CHILL:
                            DoCastVictim(SPELL_CHILL);
                            events.ScheduleEvent(EVENT_CHILL, urand(13000, 20000));
                            break;
                        case EVENT_BREATH:
                            DoCastVictim(SPELL_FROST_BREATH);
                            if (_enraged)
                                events.ScheduleEvent(EVENT_BREATH, urand(4000, 8000));
                            else
                                events.ScheduleEvent(EVENT_BREATH, urand(8000, 12000));
                            break;
                        case EVENT_TELEPORT:
                        {
                            Talk(SAY_TELEPORT);
                            ...
                            for (ThreatContainer::StorageType::const_iterator i = threatlist.begin(); i != threatlist.end(); ++i)
                            {
                                if (Player* player = ObjectAccessor::GetPlayer(*me, (*i)->getUnitGuid()))
                                    DoTeleportPlayer(player, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ()+10, player->GetOrientation());
                            }
                            events.ScheduleEvent(EVENT_TELEPORT, 30000);
                            break;
                        }
