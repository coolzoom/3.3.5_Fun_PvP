...
public:
    ...
    struct boss_aeonusAI : public BossAI
    {
        ...
        void Reset() override 
        { 
            me->GetMotionMaster()->MoveJump(-2061.37f, 7125.80f, 30, 6, 3);
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            ...
            events.ScheduleEvent(EVENT_SANDSTORM, 30000);
                ...
                switch (eventId)
                {
                    ...
                    case EVENT_SANDSTORM:
                        ...
                        if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 30.0f, true))
                            me->SummonCreature(NPC_SANDSTORM, target->GetPositionX(), target->GetPositionY()+5, target->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 45000);
                        if (IsHeroic())
                            events.ScheduleEvent(EVENT_SANDSTORM, 15000);
                        else
                            events.ScheduleEvent(EVENT_SANDSTORM, 30000);
                        break;
                    default:
