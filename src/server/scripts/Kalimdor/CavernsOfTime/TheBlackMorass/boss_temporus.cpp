...
public:
    ...
    struct boss_temporusAI : public BossAI
    {
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            ...
            events.ScheduleEvent(EVENT_WING_BUFFET, urand(9000, 10000));
            if (IsHeroic())
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_WING_BUFFET:
                        if (IsHeroic())
                            DoCast(me, H_SPELL_WING_BUFFET);
                        else
                            DoCast(me, SPELL_WING_BUFFET);
                        events.ScheduleEvent(EVENT_WING_BUFFET, urand(8000, 20000));
                        break;
                    case EVENT_SPELL_REFLECTION: // Only in Heroic
                        DoCast(me, SPELL_REFLECT);
                        events.ScheduleEvent(EVENT_SPELL_REFLECTION, urand(15000, 35000));
                        break;
                    default:
                        break;
                }
