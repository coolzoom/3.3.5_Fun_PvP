...
class boss_magtheridon : public CreatureScript
{
    public:
        ...
        struct boss_magtheridonAI : public ScriptedAI
        {
            ...
            void UpdateAI(uint32 diff) override
            {
                if (!me->HasAura(SPELL_SHADOW_GRASP_C))
                {
                    me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                ...
                if (Fireball_Timer <= diff)
                {
                    if (!me->HasUnitState(UNIT_STATE_STUNNED) && !me->HasUnitState(UNIT_STATE_CASTING))
                        DoCast(SelectTarget(SELECT_TARGET_RANDOM, 0), SPELL_FEL_FIREBALL, true);
                ...
                if (Blaze_Timer <= diff)
                {
                    if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 70, true)) // only on players.
                    {
                        float x, y, z;
                        target->GetPosition(x, y, z);
                        Creature* summon = me->SummonCreature(NPC_ABYSSAL, x, y, z, 0, TEMPSUMMON_CORPSE_DESPAWN, 180000);
                        if (summon)
                        {
                    ...
                    Blaze_Timer = urand(20000, 30000);
                }
                else
                    Blaze_Timer -= diff;

                if (!Phase3 && HealthBelowPct(50)
    ...
    public:
        ...
        struct npc_hellfire_channelerAI : public ScriptedAI
        {
            ...
            void Initialize()
            {
                ...
                Infernal_Timer = urand(45000, 50000);
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                if (Infernal_Timer <= diff)
                {
                    ...
                    Infernal_Timer = urand(60000, 90000);
                }
                else
                    Infernal_Timer -= diff;
...
public:
    ...
    bool OnGossipHello(Player* player, GameObject* go) override
    {
        ...
        if (Creature* pTrigger = go->FindNearestCreature(17376, 5.0f))
