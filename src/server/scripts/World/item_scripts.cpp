...
/*class item_custom_chest : public ItemScript
{
public:
    item_custom_chest() : ItemScript("item_custom_chest") { }

    bool OnUse(Player* player, Item* item, SpellCastTargets const& /*targets*//*) override
    {
        if (!player->HasItemCount(42482))
        {
            ChatHandler(player->GetSession()).PSendSysMessage("|cffff0000Required case unlocker to open. |r");
            player->GetSpellHistory()->AddCooldown(73213, item->GetEntry(), std::chrono::seconds(2));
            return false; 
        }
        ...
        switch (itemId)
        {
            case 4632:
                if (irand(1,100) < 30)
                    result = WorldDatabase.Query("SELECT entry FROM item_template WHERE itemset = 0 AND ItemLevel > 308 AND ItemLevel < 326 AND Quality=4 ORDER BY RAND() LIMIT 5");
                else if (irand(1,100) < 50)
                    result = WorldDatabase.Query("SELECT entry FROM item_template WHERE itemset = 0 AND ItemLevel > 294 AND ItemLevel < 318 AND Quality=4 ORDER BY RAND() LIMIT 5");
                else
                    result = WorldDatabase.Query("SELECT entry FROM item_template WHERE itemset = 0 AND ItemLevel > 284 AND ItemLevel < 308 AND Quality=4 ORDER BY RAND() LIMIT 5");
                break;
            case 4633:
                if (irand(1,100) < 50)
                    result = WorldDatabase.Query("SELECT entry FROM item_template WHERE itemset = 0 AND ItemLevel > 294 AND ItemLevel < 326 AND Quality=4 ORDER BY RAND() LIMIT 5");
                else
                    result = WorldDatabase.Query("SELECT entry FROM item_template WHERE itemset = 0 AND ItemLevel > 284 AND ItemLevel < 318 AND Quality=4 ORDER BY RAND() LIMIT 5");
                break;
            default: break;
        }
        ...
        if (!result)
        {
            TC_LOG_INFO("server.loading", "Item Template table is empty");
            return false;
        }
        do
        {
            Field* fields = result->Fetch();
            qEntry  = fields[0].GetUInt32();

            if (player->HasItemCount(qEntry))
                continue;
        ...
        //if (ItemTemplate const* temp = sObjectMgr->GetItemTemplate(newItem))
        //    sWorld->SendWorldText(LANG_ANNOUNCE_CHEST_WIN, player->GetName().c_str(), temp->Name1.c_str());
        
        uint32 noSpaceForCount = 0;
        ItemPosCountVec dest;
        InventoryResult msg = player->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, newItem, 1, &noSpaceForCount);
        if (msg != EQUIP_ERR_OK)
            return false;
            
        Item* item = player->StoreNewItem(dest, newItem, true, Item::GenerateItemRandomPropertyId(newItem));
        if (item)
        {
