...
const uint32 TransmogDisplayVendorMgr::TokenEntry = 34628;
...
const char* TransmogDisplayVendorMgr::getSlotName(uint8 slot, WorldSession* /*session*/)
{
    ...
    switch (slot)
    {
        ...
        case EQUIPMENT_SLOT_WAIST     : return  "Ремень";// session->GetTrinityString(LANG_SLOT_NAME_WAIST);
        ...
        case EQUIPMENT_SLOT_WRISTS    : return  "Наручи";// session->GetTrinityString(LANG_SLOT_NAME_WRISTS);
        case EQUIPMENT_SLOT_HANDS     : return  "Руки";// session->GetTrinityString(LANG_SLOT_NAME_HANDS);
        case EQUIPMENT_SLOT_BACK      : return  "Спина";// session->GetTrinityString(LANG_SLOT_NAME_BACK);
        case EQUIPMENT_SLOT_MAINHAND  : return  "Основное оружие";// session->GetTrinityString(LANG_SLOT_NAME_MAINHAND);
        case EQUIPMENT_SLOT_OFFHAND   : return  "Вторичное оружие";// session->GetTrinityString(LANG_SLOT_NAME_OFFHAND);
        case EQUIPMENT_SLOT_RANGED    : return  "Оружие дальнего боя";// session->GetTrinityString(LANG_SLOT_NAME_RANGED);
        case EQUIPMENT_SLOT_TABARD    : return  "Накидка";// session->GetTrinityString(LANG_SLOT_NAME_TABARD);
        default: return NULL;
    }
}
    ...
    else
    {
        ...
        if (!no_cost)
        {
            if (RequireToken)
            {
                ...
                {
                    player->GetSession()->SendNotification("У вас недостаточно %ss", getItemName(sObjectMgr->GetItemTemplate(TransmogDisplayVendorMgr::TokenEntry), player->GetSession()).c_str());
        ...
        player->GetSession()->SendAreaTriggerMessage("%s трансмогрифицирован", slotname);
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TRAINER, "Удалить трансмогрификацию", SENDER_REMOVE_MENU, 0);
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action) override
    {
        ...
        switch (sender)
        {
            case SENDER_SELECT_VENDOR: // action = slot
            {
                ...
                if (!item)
                {
                    if (const char* slotname = TransmogDisplayVendorMgr::getSlotName(action, player->GetSession()))
                        session->SendNotification("Не найдено предметов в слоте: %s ", slotname);
                ...
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, "Назад..", SENDER_BACK, 0);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
            ...
            case SENDER_REMOVE_ALL: // Remove TransmogDisplayVendorMgrs
            {
                ...
                if (removed)
                {
                    session->SendAreaTriggerMessage("Трансмогрификация удалена");
                    player->PlayDirectSound(3337);
                }
                else
                {
            ...
            case SENDER_REMOVE_ONE: // Remove TransmogDisplayVendorMgr from single item
            {
                ...
                if (Item* newItem = player->GetItemByPos(INVENTORY_SLOT_BAG_0, action))
                {
                    if (TransmogDisplayVendorMgr::GetFakeEntry(newItem))
                    {
                        TransmogDisplayVendorMgr::DeleteFakeEntry(player, newItem);
                        if (slotname)
                            session->SendAreaTriggerMessage("%s трансмогрификация удалена", slotname);
                        player->PlayDirectSound(3337);
                    }
                    else if (slotname)
                    {
            ...
            case SENDER_REMOVE_MENU:
            {
                for (uint8 slot = EQUIPMENT_SLOT_START; slot < EQUIPMENT_SLOT_END; slot++)
                {
                    ...
                    player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_INTERACT_1, (std::string)"Удалить с  " + slotname, SENDER_REMOVE_ONE, slot, ss.str().c_str(), 0, false);
                }
                player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_INTERACT_1, "Удалить трансмогрификацию", SENDER_REMOVE_ALL, 0, "Вы действительно хотите удалить всю трансмогрификацию?", 0, false);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, "Назад..", SENDER_BACK, 0);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
