...
Position const SpawnLoc = { 468.931f, -513.555f, 104.723f, 0 };
Position const firstWave[] =
{
    // Warriors
    ...
    struct boss_skadiAI : public ScriptedAI
    {
        ...
        void SummonedCreatureDespawn(Creature* summoned) override
        {
            if (summoned->GetEntry() == NPC_GRAUF)
                m_uiGraufGUID = ObjectGuid::Empty;
        ...
        void DoAction(int32 action) override
        {
            switch (action)
            {
                ...
                case DATA_GAUNTLET_END:
                    ...
                    me->GetMotionMaster()->MoveJump(Location[4], 25.0f, 5.0f);
                ...
                case PHASE_FLYING:
                    ...
                    switch (uint32 eventId = events.ExecuteEvent())
                    {
                        ...
                        case EVENT_SPAWN_MOVE:
                            if (!spawns.empty())
                            {
                                std::list<Creature*>::const_iterator itr = spawns.begin();
                                for (itr; itr != spawns.end(); ++itr)
                ...
                case EVENT_EMOTE:
                    switch (step)
                    {
                        case 2:
                            ...
                            if (!triggers.empty())
                            {
                                std::list<Creature*>::const_iterator itr = triggers.begin();
                                for (itr; itr != triggers.end(); ++itr)
                                    (*itr)->AI()->SetData(0, 0);
                            }
                            break;
                        case 3:
