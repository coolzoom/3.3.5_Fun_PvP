...
enum Enums
{
    ...
    SPELL_FLAME_TSUNAMI_DMG_TRIGGER             = 57492,    // periodic damage, npc has this aura
...
public:
    ...
    struct npc_flame_tsunamiAI : public ScriptedAI
    {
        npc_flame_tsunamiAI(Creature* creature) : ScriptedAI(creature)
        {
            ...
            me->AddAura(SPELL_FLAME_TSUNAMI_DMG_TRIGGER, me);
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_TSUNAMI_BUFF:
                        ...
                        // Finding players in range to knock them
                        ...
                        for (std::list<Unit*>::const_iterator iter = targets.begin(); iter != targets.end(); ++iter)
                             if ((*iter)->IsAlive())
                                (*iter)->CastSpell((*iter), SPELL_FLAME_TSUNAMI_LEAP, true);
...
class go_twilight_portal_obs : public GameObjectScript
{
    public:
        go_twilight_portal_obs() : GameObjectScript("go_twilight_portal_obs") { }

            bool OnGossipHello(Player* player, GameObject* go)
            {
                player->CastSpell(player, 57620, true);
                return true;
            }
};
