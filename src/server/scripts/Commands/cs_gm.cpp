...
public:
    ...
    static bool HandleGMListIngameCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        for (HashMapHolder<Player>::MapType::const_iterator itr = m.begin(); itr != m.end(); ++itr)
        {
            ...
            if (itrSec == 0) // Prevent showing arena spectators
                continue;
                ...
                (!handler->GetSession() || itr->second->IsVisibleGloballyFor(handler->GetSession()->GetPlayer())))
            {
                ...
                if (handler->GetSession())
                    handler->PSendSysMessage("|    Game Master: %s, GMLevel: %u |", handler->playerLink(itr->second->GetName()).c_str(), security);
                else
        ...
        if (result)
        {
            ...
            do
            {
                ...
                if ((max + max2 + strlen(name)) == 16)
                    max2 = max - 1;
                if (handler->GetSession())
                    handler->PSendSysMessage("|    GM Account: %s, GMLevel: %u |", name, security);
                else
