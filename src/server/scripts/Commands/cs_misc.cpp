...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        ...
        {
            { "add",                rbac::RBAC_PERM_COMMAND_BADWORD_ADD,      true,  &HandleBadWordAddCommand,           "" },
            { "remove",             rbac::RBAC_PERM_COMMAND_BADWORD_REMOVE,   true,  &HandleBadWordRemoveCommand,        "" },
            { "list",               rbac::RBAC_PERM_COMMAND_BADWORD_LIST,     true,  &HandleBadWordListCommand,          "" },
            { "mod",                rbac::RBAC_PERM_COMMAND_BADWORD_MOD,      true,  &HandleWordFilterModCommand,        "" },
        };
        ...
        static std::vector<ChatCommand> commandTable =
        {
            ...
            { "badword",          rbac::RBAC_PERM_COMMAND_BADWORD,          false, NULL,                           "", badWordCommandTable }, // rly dont know rbac system
            ...
            { "phide",            SEC_PLAYER,                               false, &HandlePhideCommand,            "" },
            { "itemlevel",        SEC_PLAYER,                               false, &HandleItemLevelCommand,        "" },
            { "set_itemlevel",    rbac::RBAC_PERM_COMMAND_ADDITEM,          false, &HandleSetItemLevelCommand,     "" },
    ...
    static bool HandleAddItemCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (count > 0 && item)
        {
			...
			/*if (ItemTemplate const* additem = sObjectMgr->GetItemTemplate(itemId))
                if (additem->Quality == ITEM_QUALITY_EPIC || additem->Quality == ITEM_QUALITY_LEGENDARY)
					player->CreateRecentActivity(2, itemId, count);*/
    ...
    */
    ...
    static bool HandlePhideCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (me->HasAura(36573))
        {
            me->RemoveAurasDueToSpell(36573);
            handler->PSendSysMessage("Hide player aura removed!");
            return true;
        }
            
        if (me->GetZoneId() != 394)
        {
            handler->PSendSysMessage("You have to be in (Grizzlemaw) Shop!");
            return false;
        }
            
        if (me->duel && me->duel->opponent)
        {
            handler->PSendSysMessage("Disabled in duels!");
            return false;
        }
            
        handler->PSendSysMessage("Your phase have been change, other players still can see you. Use command .phide for return a normal phase!");
        me->AddAura(36573, me);
        return true;
    }
    ...
    static bool HandleItemLevelCommand(ChatHandler* handler, char const*args)
    {
        std::string name;
        Player* player;
        char* targetName = strtok((char*)args, " "); // Get entered name
        ...
        else if (player = handler->getSelectedPlayer()) // If no name was entered - use target
            player = handler->getSelectedPlayer();
        else
        ...
        result << "------------------------------ \n";
        result << "Character Name: " << name.c_str() << "\n";
        result << "Average ItemLevel: " << int32(player->GetAverageItemLevel()) << "\n";
        result << "------------------------------ \n";
    ...
    static bool HandleSetItemLevelCommand(ChatHandler* handler, char const*args)
    {
        char* itemGuidStr;
        char* levelStr;
        handler->extractOptFirstArg((char*)args, &itemGuidStr, &levelStr);
        if (!levelStr || !itemGuidStr)
        {
            handler->PSendSysMessage("You must enter ItemGuid and Level!");
            return false;
        }
        uint32 guid = uint64(atoi(itemGuidStr));
        ObjectGuid itemGuid = ObjectGuid(HighGuid::Item, guid);
            
        int32 iLevel = uint32(atoi(levelStr));
        ...
        if (!itemGuid)
            handler->PSendSysMessage("WRONG ITEM GUID", itemGuidStr, iLevel);

        handler->PSendSysMessage("ItemGuid: %s, Level: %i", itemGuidStr, iLevel);
        
        if (Item* item = player->GetItemByGuid(itemGuid))
        {
            item->SetItemUpdateLevel(iLevel);
            handler->GetSession()->GetPlayer()->UpdateObjectVisibility();
            handler->PSendSysMessage("Succesful changed");
        }
    ...
    static bool HandleAddCoinCommand(ChatHandler* handler, char const* args)
    {
        ...
        QueryResult result = LoginDatabase.PQuery("SELECT coin FROM account WHERE id = '%u'", acc);
        coin += result->Fetch()->GetFloat();
        LoginDatabase.PExecute("UPDATE account set coin = %f WHERE id = '%u'", coin, acc);
        handler->PSendSysMessage("Всего coin у игрока = %f", coin);
        return true;
    }
};
