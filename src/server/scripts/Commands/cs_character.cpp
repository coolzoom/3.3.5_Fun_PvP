...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        ...
        static std::vector<ChatCommand> characterCommandTable =
        {
            ...
            { "stats",         rbac::RBAC_PERM_COMMAND_CHARACTER_STATS,           true,  &HandleCharacterStatsCommand,          "", },
        };
    ...
    */
    ...
    static bool HandleCharacterStatsCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
		...
		float ManaRegen = float(player->GetFloatValue(UNIT_FIELD_POWER_REGEN_FLAT_MODIFIER) + player->GetFloatValue(UNIT_FIELD_POWER_REGEN_INTERRUPTED_FLAT_MODIFIER) * 1.0f);
        
        std::ostringstream result;
        result << "Имя персонажа: " << characterName.c_str() << "\n";
        result << "------------------------------ \n";
        // Stats
        result << "Сила: " << int32(player->GetStat(STAT_STRENGTH)) << "\n";
        result << "Ловкость: "   << int32(player->GetStat(STAT_AGILITY))  << "\n";
        result << "Выносливость: "   << int32(player->GetStat(STAT_STAMINA))  << "\n";
        result << "Интелект: "  << int32(player->GetStat(STAT_INTELLECT)) << "\n";
        result << "Дух: "    << int32(player->GetStat(STAT_SPIRIT))   << "\n";
        result << "Броня: "     << float(player->GetArmor())             << "\n";
        result << "------------------------------ \n";
        // Resistances
        for (int i = 2; i < MAX_SPELL_SCHOOL; ++i)
            result << "Сопротивление " << i << ": " << int32(player->GetResistance(SpellSchools(i))) << "\n";
        result << "------------------------------ \n";
        result << "Сила заклинаний: "    << int32(player->GetBaseSpellPowerBonus()) << "\n";
        result << "Сила аттаки: "   << int32(player->GetUInt32Value(UNIT_FIELD_ATTACK_POWER))  << "\n";
        result << "Пробивание брони.."   << int32(player->GetRatingBonusValue(CR_ARMOR_PENETRATION))  << "\n";
		result << "Проник.Заклинаний.. "  << int32(player->GetSpellPenetration(SpellSchoolMask(SPELL_SCHOOL_MASK_MAGIC))) << "\n";
        result << "Крит:"  << float(player->GetFloatValue(PLAYER_CRIT_PERCENTAGE))  << "\n";
        result << "Меткость: "      << int32(player->GetRatingBonusValue(CR_HIT_TAKEN_SPELL))  << "\n";
        result << "Скорость: "          << float(player->GetFloatValue(UNIT_MOD_CAST_SPEED)) << "\n";
        result << "Мана реген: "            << float(ManaRegen) << "\n";
        result << "Рейтинг устойчивости: "     << int32(player->GetUInt32Value(PLAYER_FIELD_COMBAT_RATING_1 + CR_CRIT_TAKEN_SPELL))  << "\n";
        result << "------------------------------ \n";
		std::string stats = result.str();
		handler->PSendSysMessage("%s", stats);
        return true;
    }
