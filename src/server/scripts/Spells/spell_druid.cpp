                ...
                if (Unit* unitTarget = GetHitUnit())
                    if (AuraEffect const* aurEff = unitTarget->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_DRUID, 0x00000002, 0, 0, caster->GetGUID()))
                    {
                        ...
                        if (countMin < countMax)
                        {
                            ...
                            aura->SetDuration(uint32(aura->GetMaxDuration()));
                        }
                    }
            }
    ...
    public:
        ...
        class spell_dru_starfall_dummy_SpellScript : public SpellScript
        {
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                ...
                if (GetHitUnit() && GetHitUnit()->HasBreakableByDamageCrowdControlAura())
                    return;
...
class spell_pet_treant : public CreatureScript
{
    public:
        spell_pet_treant() : CreatureScript("spell_pet_treant") { }

		struct spell_pet_treantAI : ScriptedAI
		{
			spell_pet_treantAI(Creature* creature) : ScriptedAI(creature) { }            
			...
			void Reset() override
			{
                ...
                _events.ScheduleEvent(TREANT_DAZE_EVENT, 0);
			...
			void UpdateAI(uint32 diff) override
			{
                ...
                if (_events.ExecuteEvent() == TREANT_DAZE_EVENT)
                {
                    DoCastVictim(SPELL_DRUID_TREANT_DAZE);
                    _events.ScheduleEvent(TREANT_DAZE_EVENT, urand(7500, 10000));
                }
        ...
        {
            return new spell_pet_treantAI(creature);
        }
};
